...
public partial class BRT_BuildReportWindow : EditorWindow
{
	...
	UnityEngine.Vector2 _assetListScrollPos;

	public static bool IsOpen { get; set; }

	UnityEngine.Vector2 _readmeScrollPos;
	string _readmeContents;
	float _readmeHeight;

	UnityEngine.Vector2 _changelogScrollPos;
	...
	void InitGUISkin()
	{
		...
		if (_usedSkin == null)
		{
			...
			string folderPath = BuildReportTool.Util.FindAssetFolder(UnityEngine.Application.dataPath, BuildReportTool.Options.BUILD_REPORT_TOOL_DEFAULT_FOLDER_NAME);
		...
		if (list != null)
		{
			...
			if (assetListToUse != null)
			{
				...
				else
				{
					...
					EditorGUIUtility.SetIconSize(UnityEngine.Vector2.one * ICON_DISPLAY_SIZE);
					bool useAlt = false;
					...
					int len = UnityEngine.Mathf.Min(viewOffset + length, assetListToUse.Length);
					...
					GUILayout.BeginVertical();
						...
						for (int n = viewOffset; n < len; ++n)
						{
							...
							GUILayout.BeginHorizontal();
								...
								UnityEngine.Texture icon = AssetDatabase.GetCachedIcon(b.Name);
	...
	void OnGUI()
	{
		...
		if ((IsInUsedAssetsCategory && _buildInfo.HasUsedAssets) || (IsInUnusedAssetsCategory && _buildInfo.HasUnusedAssets))
		{
			...
			GUILayout.BeginHorizontal();
				...
				int len = UnityEngine.Mathf.Min(viewOffset + BuildReportTool.Options.AssetListPaginationLength, assetListToUse.Length);
