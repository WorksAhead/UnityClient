...
public class EasyJoystick : UnityEngine.MonoBehaviour {
	...
	#region   property
	private UnityEngine.Vector2 joystickAxis;
	/// <summary>
	...
	/// The joystick axis.
	/// </value>
	public UnityEngine.Vector2 JoystickAxis {
		get {
	...
	/// </summary>
	private UnityEngine.Vector2 joystickTouch;
	public UnityEngine.Vector2 JoystickTouch {
		get {
			return new UnityEngine.Vector2(this.joystickTouch.x/(zoneRadius), this.joystickTouch.y/(zoneRadius));
		}
		set {
			float x = UnityEngine.Mathf.Clamp( value.x,-1f,1f)*(zoneRadius);
			float y = UnityEngine.Mathf.Clamp( value.y,-1f,1f)*(zoneRadius);
			joystickTouch = new UnityEngine.Vector2(x,y);
		}
	}	
	
	private UnityEngine.Vector2 joystickValue;
	/// <summary>
	...
	/// The joystick value.
	/// </value>
	public UnityEngine.Vector2 JoystickValue {
		get {
	...
	/// </summary>
	[SerializeField]
	private UnityEngine.Vector2 joystickPositionOffset = UnityEngine.Vector2.zero;
	public UnityEngine.Vector2 JoystickPositionOffset {
		get {
  ...
  public float MaxTouchRadiusOffset = 50f;
	...
	/// </summary>
	public UnityEngine.GameObject receiverGameObject; 
	...
	[SerializeField]
	private UnityEngine.Transform xAxisTransform;
	/// <summary>
	...
	/// </value>
	public UnityEngine.Transform XAxisTransform {
		get {
	...
	[SerializeField]
	private UnityEngine.Transform yAxisTransform;
	/// <summary>
	...
	/// </value>
	public UnityEngine.Transform YAxisTransform {
		get {
	...
	[SerializeField]
	public UnityEngine.Vector2 smoothing = new UnityEngine.Vector2(2f,2f);
	/// <summary>
	...
	[SerializeField]
	public UnityEngine.Vector2 inertia = new UnityEngine.Vector2(100,100);
	/// <summary>
	...
	/// The inertia.
	/// </value>
	public UnityEngine.Vector2 Inertia {
		get {
			return this.inertia;
		}
		set {
	...
	/// The area texture.
	/// </summary>
	public UnityEngine.Texture areaTexture;
	/// <summary>
	/// The color of the area.
	/// </summary>
	public UnityEngine.Color areaColor = UnityEngine.Color.white;
	/// <summary>
	/// The touch texture.
	/// </summary>
	public UnityEngine.Texture touchTexture;
	/// <summary>
	/// The color of the touch.
	/// </summary>
	public UnityEngine.Color touchColor = UnityEngine.Color.white;
	/// <summary>
	/// The dead texture.
	/// </summary>
	public UnityEngine.Texture deadTexture;
  ...
  public int joystickIndex = -1;
	...
	// Joystick properties
	private UnityEngine.Vector2 joystickCenter;
	
	private Rect areaRect;
	private Rect deadRect;
	
	private UnityEngine.Vector2 anchorPosition = UnityEngine.Vector2.zero;
	...
	void UpdateJoystick(){
		if (UnityEngine.Application.isPlaying){
			...
			if (isActivated){
				...
				if ((joystickIndex==-1) || (joystickAxis == UnityEngine.Vector2.zero && joystickIndex>-1)){
					if (enableXAutoStab){
        ...
        if (joystickIndex == -1 || Input.touchCount <= 0) {
#else
        if (joystickIndex == -1 || !Input.GetMouseButton(0)) {
#endif
					if (!enableSmoothing){
						joystickTouch = UnityEngine.Vector2.zero;
					}
					else{ 
						if (joystickTouch.sqrMagnitude>0.0001){
							joystickTouch = new UnityEngine.Vector2( joystickTouch.x - joystickTouch.x*smoothing.x*Time.deltaTime, joystickTouch.y - joystickTouch.y*smoothing.y*Time.deltaTime);	
						}
						else{
							joystickTouch = UnityEngine.Vector2.zero;
						}
				...
				UnityEngine.Vector2 oldJoystickAxis = new UnityEngine.Vector2(joystickAxis.x,joystickAxis.y);
   				float deadCoef = ComputeDeadZone();
    			joystickAxis= new UnityEngine.Vector2(joystickTouch.x*deadCoef, joystickTouch.y*deadCoef);				
				...
				// Joystick Value
				UnityEngine.Vector2 realvalue = new UnityEngine.Vector2(  speed.x*joystickAxis.x,speed.y*joystickAxis.y);
				if (enableInertia){
					UnityEngine.Vector2 tmp = (realvalue - joystickValue);
					tmp.x /= inertia.x;
					tmp.y /= inertia.y;
					joystickValue += tmp;
				...
				// Start moving
				if (oldJoystickAxis == UnityEngine.Vector2.zero && joystickAxis != UnityEngine.Vector2.zero){
					if (interaction != InteractionType.Direct && interaction != InteractionType.Include){
				...
				if (joystickAxis != UnityEngine.Vector2.zero){
					sendEnd = false;
					switch(interaction){
	...
	void OnGUI(){
		...
		if (enable){
			...
			if (dynamicJoystick && UnityEngine.Application.isEditor && !UnityEngine.Application.isPlaying){
				switch (area){
			...
			if ((showZone && areaTexture!=null && !dynamicJoystick) || (showZone && dynamicJoystick && virtualJoystick && areaTexture!=null) 
				|| (dynamicJoystick && UnityEngine.Application.isEditor && !UnityEngine.Application.isPlaying)){
				if (isActivated){
					GUI.color = areaColor;
					
					if (UnityEngine.Application.isPlaying && !dynamicJoystick){
						EasyTouch.RemoveReservedArea( areaRect );
						EasyTouch.AddReservedArea( areaRect );
					}
				}
				else{
					GUI.color = new UnityEngine.Color(areaColor.r,areaColor.g,areaColor.b,0.2f);	
					if (UnityEngine.Application.isPlaying && !dynamicJoystick){
						EasyTouch.RemoveReservedArea( areaRect );	
					}
				}		
				
				if (showDebugRadius && UnityEngine.Application.isEditor){
					GUI.Box( areaRect,"");				
				}
			...
			if ((showTouch && touchTexture!=null && !dynamicJoystick)|| (showTouch && dynamicJoystick && virtualJoystick && touchTexture!=null) 
				...
				if (isActivated){
					...
					GUI.color = new UnityEngine.Color(touchColor.r,touchColor.g,touchColor.b,0.2f);	
				}			
  ...
  void UpdateDirect()
  {
		...
		if (xAxisTransform !=null){
			// Axis influenced
			UnityEngine.Vector3 axis =GetInfluencedAxis( xAI);
			// Action
		...
		if (YAxisTransform !=null){
			// Axis
			UnityEngine.Vector3 axis = GetInfluencedAxis(yAI);
			// Action
	...
	void UpdateGravity(){
		...
		if (xAxisCharacterController!=null && xAxisGravity>0){
			xAxisCharacterController.Move( UnityEngine.Vector3.down*xAxisGravity*Time.deltaTime);	
		}
			
		if (yAxisCharacterController!=null && yAxisGravity>0){
			yAxisCharacterController.Move( UnityEngine.Vector3.down*yAxisGravity*Time.deltaTime);
		}		
	}
	
	UnityEngine.Vector3 GetInfluencedAxis(AxisInfluenced axisInfluenced){
	...
	void DoActionDirect(UnityEngine.Transform axisTransform, PropertiesInfluenced inlfuencedProperty,UnityEngine.Vector3 axis, float sensibility, CharacterController charact){
	...
	void DoAngleLimitation(UnityEngine.Transform axisTransform, AxisInfluenced axisInfluenced,float clampMin, float clampMax,float startAngle){
		...
		//angle = UnityEngine.Mathf.Clamp (angle, startAngle-clampMax, clampMin+startAngle);
		angle = UnityEngine.Mathf.Clamp (angle, -clampMax, clampMin);
		
		
		switch(axisInfluenced){
			case AxisInfluenced.X:
				axisTransform.localEulerAngles = new UnityEngine.Vector3( angle,axisTransform.localEulerAngles.y, axisTransform.localEulerAngles.z);
				break;	
			case AxisInfluenced.Y:
				axisTransform.localEulerAngles = new UnityEngine.Vector3( axisTransform.localEulerAngles.x,angle, axisTransform.localEulerAngles.z);
				break;
			case AxisInfluenced.Z:
				axisTransform.localEulerAngles = new UnityEngine.Vector3( axisTransform.localEulerAngles.x,axisTransform.localEulerAngles.y,angle);
				break;	
	...
	void DoAutoStabilisation(UnityEngine.Transform axisTransform, AxisInfluenced axisInfluenced, float threshold, float speed,float startAngle){
		...
		if (angle > startAngle - threshold || angle < startAngle + threshold){
			...
			float axis=0;
			UnityEngine.Vector3 stabAngle = UnityEngine.Vector3.zero;
			
			if (angle > startAngle - threshold){
				axis = angle + speed/100f*UnityEngine.Mathf.Abs (angle-startAngle) * Time.deltaTime*-1;
			}
		
		
			if (angle < startAngle + threshold){
				axis = angle + speed/100f*UnityEngine.Mathf.Abs (angle-startAngle) * Time.deltaTime;
			}
		
			switch(axisInfluenced){
				case AxisInfluenced.X:	
					stabAngle = new UnityEngine.Vector3(axis,axisTransform.localRotation.eulerAngles.y,axisTransform.localRotation.eulerAngles.z);
					break;
				case AxisInfluenced.Y:	
					stabAngle = new UnityEngine.Vector3(axisTransform.localRotation.eulerAngles.x,axis,axisTransform.localRotation.eulerAngles.z);
					break;
				case AxisInfluenced.Z:	
					stabAngle = new UnityEngine.Vector3(axisTransform.localRotation.eulerAngles.x,axisTransform.localRotation.eulerAngles.y,axis);
					break;
			}
			
			axisTransform.localRotation  = UnityEngine.Quaternion.Euler( stabAngle);	
		}
	}
	
	float GetStartAutoStabAngle(UnityEngine.Transform axisTransform, AxisInfluenced axisInfluenced){
	...
	float ComputeDeadZone(){
		...
		float dead = 0;
		float dist = UnityEngine.Mathf.Max(joystickTouch.magnitude,0.1f);
		
		if (restrictArea){
			dead = UnityEngine.Mathf.Max(dist - deadZone, 0)/(zoneRadius-touchSize - deadZone)/dist;
		}
		else{
			dead = UnityEngine.Mathf.Max(dist - deadZone, 0)/(zoneRadius - deadZone)/dist;
		}		
	...
	void ComputeJoystickAnchor(JoystickAnchor anchor){
		...
		switch (anchor){
			case JoystickAnchor.UpperLeft:
				anchorPosition = new UnityEngine.Vector2( zoneRadius+touch, zoneRadius+touch);
				break;
			case JoystickAnchor.UpperCenter:
				anchorPosition = new UnityEngine.Vector2( VirtualScreen.width/2, zoneRadius+touch);
				break;
			case JoystickAnchor.UpperRight:
				anchorPosition = new UnityEngine.Vector2( VirtualScreen.width-zoneRadius-touch,zoneRadius+touch);
				break;
			
			
			case JoystickAnchor.MiddleLeft:
				anchorPosition = new UnityEngine.Vector2( zoneRadius+touch, VirtualScreen.height/2);
				break;
			case JoystickAnchor.MiddleCenter:
				anchorPosition = new UnityEngine.Vector2( VirtualScreen.width/2, VirtualScreen.height/2);
				break;
			case JoystickAnchor.MiddleRight:
				anchorPosition = new UnityEngine.Vector2( VirtualScreen.width-zoneRadius-touch,VirtualScreen.height/2);
				break;
			
			case JoystickAnchor.LowerLeft:
				anchorPosition = new UnityEngine.Vector2( zoneRadius+touch, VirtualScreen.height-zoneRadius-touch);
				break;
			case JoystickAnchor.LowerCenter:
				anchorPosition = new UnityEngine.Vector2( VirtualScreen.width/2, VirtualScreen.height-zoneRadius-touch);
				break;
			case JoystickAnchor.LowerRight:
				anchorPosition = new UnityEngine.Vector2( VirtualScreen.width-zoneRadius-touch,VirtualScreen.height-zoneRadius-touch);
				break;	
			
			case JoystickAnchor.None:
				anchorPosition = UnityEngine.Vector2.zero;
				break;
		}
  ...
  void On_TouchStart(EasyTouchGesture gesture)
  {
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick){
			...
			if (isActivated){
				if (!dynamicJoystick){
          if (-1 == joystickIndex) {
            UnityEngine.Vector2 center = new UnityEngine.Vector2((anchorPosition.x + joystickCenter.x) * VirtualScreen.xRatio, (VirtualScreen.height - anchorPosition.y - joystickCenter.y) * VirtualScreen.yRatio);
            ...
            //if (gesture.position.y < Screen.height / 2 && gesture.position.x < Screen.width / 2) {
              ...
              CreateEvent(MessageName.On_JoystickTouchStart);
					...
					if (!virtualJoystick){
						...
						if (virtualJoystick){
							joystickCenter =new UnityEngine.Vector2(gesture.position.x/VirtualScreen.xRatio,  VirtualScreen.height - gesture.position.y/VirtualScreen.yRatio);
  ...
  void On_TouchDown(EasyTouchGesture gesture)
  {
		...
		if ((!gesture.isHoverReservedArea && dynamicJoystick) || !dynamicJoystick){
          ...
          //if (((gesture.position.y < Screen.height / 2 && gesture.position.x < Screen.width / 2) && resetFingerExit) || !resetFingerExit) {
            joystickTouch = new UnityEngine.Vector2(gesture.position.x, gesture.position.y) - center;
            joystickTouch = new UnityEngine.Vector2(joystickTouch.x / VirtualScreen.xRatio, joystickTouch.y / VirtualScreen.yRatio);
