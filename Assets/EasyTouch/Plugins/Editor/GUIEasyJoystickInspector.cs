      ...
      t.MaxTouchRadiusOffset = EditorGUILayout.FloatField("Touch Radius Offset", t.MaxTouchRadiusOffset);
		...
		if (t.showInteraction){
			...
			if (t.Interaction == EasyJoystick.InteractionType.EventNotification || t.Interaction == EasyJoystick.InteractionType.DirectAndEvent){
				...
				if (t.useBroadcast){
					t.receiverGameObject =(UnityEngine.GameObject) EditorGUILayout.ObjectField("    Receiver gameobject",t.receiverGameObject,typeof(UnityEngine.GameObject),true);
			...
			// X axis
			GUI.color = new UnityEngine.Color(255f/255f,69f/255f,40f/255f);
			...
			if (t.enableXaxis){
				...
				if (t.Interaction == EasyJoystick.InteractionType.Direct || t.Interaction == EasyJoystick.InteractionType.DirectAndEvent){
					t.XAxisTransform = (UnityEngine.Transform)EditorGUILayout.ObjectField("Joystick X to",t.XAxisTransform,typeof(UnityEngine.Transform),true);
					if ( t.XAxisTransform!=null){
						...
						case EasyJoystick.AxisInfluenced.Z:
							GUI.color = new UnityEngine.Color(63f/255f,131f/255f,245f/255f);
							break;
						}
			...
			if (t.enableYaxis){
				...
				if (t.Interaction == EasyJoystick.InteractionType.Direct || t.Interaction == EasyJoystick.InteractionType.DirectAndEvent){
					t.YAxisTransform = (UnityEngine.Transform)EditorGUILayout.ObjectField("Joystick Y to",t.YAxisTransform,typeof(UnityEngine.Transform),true);
		...
		if (t.showAppearance){
			...
			if (t.showZone){ 
				...
				t.areaTexture = (UnityEngine.Texture)EditorGUILayout.ObjectField("Area texture",t.areaTexture,typeof(UnityEngine.Texture),true);
			}
			EditorGUILayout.Separator();
			...
			if (t.showTouch){
				...
				t.touchTexture = (UnityEngine.Texture)EditorGUILayout.ObjectField("Area texture",t.touchTexture,typeof(UnityEngine.Texture),true);
			}
			EditorGUILayout.Separator();
			
			t.showDeadZone = EditorGUILayout.Toggle("Show dead",t.showDeadZone);
			if (t.showDeadZone){
				t.deadTexture = (UnityEngine.Texture)EditorGUILayout.ObjectField("Dead zone texture",t.deadTexture,typeof(UnityEngine.Texture),true);
			}
