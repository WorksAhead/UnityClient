	...
	public override void OnInspectorGUI(){
		...
		if (t.showInspectorEvent){
			...
			if (t.interaction == EasyButton.InteractionType.Event){
				...
				if (t.useBroadcast){
					EditorGUILayout.BeginVertical(paddingStyle1);
					t.receiverGameObject =(UnityEngine.GameObject) EditorGUILayout.ObjectField("Receiver object",t.receiverGameObject,typeof(UnityEngine.GameObject),true);
		...
		if (t.showInspectorTexture){
			...
			t.NormalTexture = (Texture2D)EditorGUILayout.ObjectField("Normal texture",t.NormalTexture,typeof(UnityEngine.Texture),true);
			EditorGUILayout.Separator();
			t.buttonActiveColor = EditorGUILayout.ColorField("Color",t.buttonActiveColor);
			t.ActiveTexture = (Texture2D)EditorGUILayout.ObjectField("Normal texture",t.ActiveTexture,typeof(UnityEngine.Texture),true);
			EditorGUILayout.EndVertical();
