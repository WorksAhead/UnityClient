...
public class HTEditorToolKit{
	...
	public static void DrawTitle(string text){
		...
		UnityEngine.Color textColor=UnityEngine.Color.black;
		if (EditorGUIUtility.isProSkin){
			textColor = new UnityEngine.Color( 242f/255f,152f/255f,13f/255f);	
		}	
	...
	public static bool DrawTitleFoldOut( bool foldOut,string text){
		...
		UnityEngine.Color foldTextColor=UnityEngine.Color.black;
		if (EditorGUIUtility.isProSkin){
			foldTextColor = new UnityEngine.Color( 242f/255f,152f/255f,13f/255f);	
		}
	...
	private static Rect DrawTitleGradient()
	{
		...
		GUI.color = new UnityEngine.Color(0.54f,0.54f,0.54f);
	...
	private static Texture2D GetColorTexture(UnityEngine.Color color){
		Texture2D myTexture = new Texture2D(1, 16);
		myTexture.name = ("UnityEngine.Color UnityEngine.Texture by Hedgehog Team");
	...
	{
		Texture2D myTexture = new Texture2D(1, 16);
		myTexture.name = ("Gradient UnityEngine.Texture by Hedgehog Team");
		myTexture.hideFlags = HideFlags.HideInInspector;
		myTexture.filterMode = FilterMode.Bilinear;
		myTexture.hideFlags = HideFlags.DontSave;
		float start=0.4f;
		...
		UnityEngine.Color color = new UnityEngine.Color(start,start,start);
		
		UnityEngine.Color pixColor = color;
		for (int i = 0; i < 16; i++)
		{
			pixColor = new UnityEngine.Color (pixColor.r+step, pixColor.b+step, pixColor.b+step,0.5f);
	...
	private static Texture2D CreateChildGradientTexture( )
	{
		...
		for (int i = 0; i < 16; i++)
		{
			pixColor = new UnityEngine.Color (pixColor.r+step, pixColor.b+step, pixColor.b+step,0.2f);
			myTexture.SetPixel(0, i, pixColor);
		}
		myTexture.Apply();
