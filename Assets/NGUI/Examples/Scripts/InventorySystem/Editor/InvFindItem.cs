...
public class InvFindItem : ScriptableWizard
{
	...
	void FindAllByName (string partial)
	{
		...
		for (int i=0; i<InvDatabase.list.Length; i++)
		{
			for (int j=0; j<InvDatabase.list[i].items.Count; j++)
			{
				if (InvDatabase.list[i].items[j].name.Equals(partial, System.StringComparison.OrdinalIgnoreCase))
				{
					FindResult fr = new FindResult();
					fr.db = InvDatabase.list[i];
					fr.item = InvDatabase.list[i].items[j];
					mResults.Add(fr);
				}
			}
		}
		...
		for (int i=0; i<InvDatabase.list.Length; i++)
		{
			for (int j=0; j<InvDatabase.list[i].items.Count; j++)
			{
				if (InvDatabase.list[i].items[j].name.StartsWith(partial, System.StringComparison.OrdinalIgnoreCase))
				{
					bool exists = false;
					for (int k=0; k<mResults.Count; k++)
					{
						if (mResults[k].item == InvDatabase.list[i].items[j])
						{
							exists = true;
							break;
						}
		...
		for (int i=0; i<InvDatabase.list.Length; i++)
		{
			for (int j=0; j<InvDatabase.list[i].items.Count; j++)
			{
				if (InvDatabase.list[i].items[j].name.ToLower().Contains(partial))
				{
					bool exists = false;
					for (int k=0; k<mResults.Count; k++)
					{
