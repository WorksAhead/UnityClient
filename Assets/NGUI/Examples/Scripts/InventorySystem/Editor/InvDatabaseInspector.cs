...
public class InvDatabaseInspector : Editor
{
	...
	public Rect DrawSprite (Texture2D tex, Rect sprite, Material mat, bool addPadding, int maxSize)
	{
		...
		if (maxSize > 0)
		{
			float dim = maxSize / UnityEngine.Mathf.Max(rect.width, rect.height);
			rect.width *= dim;
			rect.height *= dim;
		}
	...
	public override void OnInspectorGUI ()
	{
		...
		{
			mIndex = UnityEngine.Mathf.Clamp(mIndex, 0, db.items.Count - 1);
		...
		else
		{
			...
			if (item != null)
			{
				...
				GUILayout.BeginHorizontal();
				{
					if (mIndex == 0) GUI.color = UnityEngine.Color.grey;
					...
					if (mIndex + 1 == db.items.Count) GUI.color = UnityEngine.Color.grey;
					if (GUILayout.Button(">>")) { mConfirmDelete = false; ++mIndex; }
					GUI.color = UnityEngine.Color.white;
				}
				GUILayout.EndHorizontal();
				...
				// Game UnityEngine.Object attachment field, left of the icon
				GUILayout.BeginHorizontal();
				UnityEngine.GameObject go = (UnityEngine.GameObject)EditorGUILayout.ObjectField("Attachment", item.attachment, typeof(UnityEngine.GameObject), false);
				if (drawIcon) GUILayout.Space(iconSize);
				GUILayout.EndHorizontal();

				// UnityEngine.Color tint field, left of the icon
				GUILayout.BeginHorizontal();
				UnityEngine.Color color = EditorGUILayout.ColorField("Color", item.color);
				if (drawIcon) GUILayout.Space(iconSize);
				GUILayout.EndHorizontal();
				...
				if (drawIcon)
				{
					extraSpace = UnityEngine.Mathf.Max(0f, extraSpace - 60f);
				...
				if (item.stats != null)
				{
					for (int i = 0; i < item.stats.Count; ++i)
					{
						...
						GUILayout.BeginHorizontal();
						{
							...
							// UnityEngine.Color the field red if it's negative, green if it's positive
							if (stat.amount > 0) GUI.backgroundColor = UnityEngine.Color.green;
							else if (stat.amount < 0) GUI.backgroundColor = UnityEngine.Color.red;
