	...
	void Start ()
	{
		if (template != null)
		{
			...
			for (int y = 0; y < maxRows; ++y)
			{
				...
				{
					UnityEngine.GameObject go = NGUITools.AddChild(gameObject, template);
					UnityEngine.Transform t = go.transform;
					t.localPosition = new UnityEngine.Vector3(padding + (x + 0.5f) * spacing, -padding - (y + 0.5f) * spacing, 0f);
					...
					b.Encapsulate(new UnityEngine.Vector3(padding * 2f + (x + 1) * spacing, -padding * 2f - (y + 1) * spacing, 0f));
