...
namespace ArkCrossEngine
{
    ...
    public class ResCacheProvider
    {
        ...
        public void SearchCacheDataByLevelId(int levelId, ref Dictionary<int, ResCacheData> container, out ResCacheData levelCacheData)
        {
            levelCacheData = GetDataByLevelId(levelId);
            if (levelCacheData != null)
            {
                SearchCacheDataRecursively(levelCacheData, ref container);
            }
        }
        private bool SearchCacheDataRecursively(ResCacheData cacheData, ref Dictionary<int, ResCacheData> container)
        {
                ...
                foreach (int link in cacheData.m_LinkList)
                {
                    try
                    {
                        ...
                        if (m_ConfigDict.ContainsKey(resId) && !container.ContainsKey(resId))
                        {
                            ...
                            {
                                SearchCacheDataRecursively(childCacheData, ref container);
                            }
                        }
                    }
                    catch (System.Exception ex)
