...
[ExecuteInEditMode]
public class SplineShaper : UnityEngine.MonoBehaviour
{
    ...
    /// </summary>
    public UnityEngine.AnimationCurve RadiusModifierCurve = UnityEngine.AnimationCurve.Linear(0, 0, 1, 1);
    ...
    /// Z modifier curve
    /// </summary>
    public UnityEngine.AnimationCurve ZModifierCurve = UnityEngine.AnimationCurve.Linear(0, 0, 1, 1);
    ...
    /// <summary>
    /// The Spline of this UnityEngine.GameObject
    /// </summary>
    ...
    public void Reset()
    {
        ...
        RadiusModifierCurve = UnityEngine.AnimationCurve.Linear(0, 0, 1, 0);
        Z = 0;
        ZModifier = ModifierMode.None;
        ZModifierCurve = UnityEngine.AnimationCurve.Linear(0, 0, 1, 0);
        Z = 0;
        m = 0;
        n1 = 1;
        n2 = 3;
    ...
    {
        mNeedRefresh = false;
        Resolution = UnityEngine.Mathf.Max(2, Resolution);
        int cpCount = mSpline.ControlPointCount;
        int cpIdx = 0;
        float dmax = Range * UnityEngine.Mathf.PI * 2;
        float dtf = (dmax) / (float)Resolution;

        double r = 0;
        float treshold2 = WeldThreshold * WeldThreshold;
        UnityEngine.Vector3 lastp = new UnityEngine.Vector3(float.MaxValue, float.MaxValue, float.MaxValue);
        UnityEngine.Vector3 p = UnityEngine.Vector3.zero;
        ...
        for (float dt = 0; dt < dmax; dt += dtf) {
            ...
            r = System.Math.Pow((UnityEngine.Mathf.Pow(UnityEngine.Mathf.Abs(UnityEngine.Mathf.Cos(m * dt / 4.0f) / a), n2) + System.Math.Pow(UnityEngine.Mathf.Abs(UnityEngine.Mathf.Sin(m * dt / 4.0f) / b), n3)), -(1 / n1));

            p.x = (float)(r * UnityEngine.Mathf.Cos(dt) * curRadius);
            p.y = (float)(r * UnityEngine.Mathf.Sin(dt) * curRadius);
            p.z = curZ;
