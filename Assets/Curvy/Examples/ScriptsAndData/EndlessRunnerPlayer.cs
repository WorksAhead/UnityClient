...
[ExecuteInEditMode]
public class EndlessRunnerPlayer : UnityEngine.MonoBehaviour {
    ...
    public UnityEngine.AnimationCurve JumpCurve = UnityEngine.AnimationCurve.EaseInOut(0, 1,1,0.3f); // Jump Curve
    ...
    UnityEngine.Vector3 mLastPosOnSpline;
	...
	void Update () 
    {
        ...
        UnityEngine.Vector3 newPosOnSpline=Spline.MoveBy(ref TF, ref dir, Speed*Time.deltaTime, CurvyClamping.PingPong);
        UnityEngine.Vector3 newTangent = Spline.GetTangent(TF);
        ...
        if (Input.GetButtonDown("Horizontal")) {
            if (TrySwitchLane(Input.GetAxis("Horizontal") < 0 ? UnityEngine.Vector3.left : UnityEngine.Vector3.right)) {
                newPosOnSpline = Spline.Interpolate(TF);
            }
        }        
        // *** Jump? ***
        if (!mInAir && !Jumping && Input.GetKey(UnityEngine.KeyCode.Space)) {
            StartCoroutine(Jump());
        }
        ...
        if (dir != 1) {
            ...
            float newTF;
            if (RaycastForFollowUpSpline(UnityEngine.Vector3.down, out newSpline, out newTF)) {
        ...
        if (!Jumping) {
            UnityEngine.Vector3 offset = mTransform.position - newPosOnSpline;
    ...
    bool TrySwitchLane(UnityEngine.Vector3 dir)
    {
    ...
    bool RaycastForFollowUpSpline(UnityEngine.Vector3 dir, out CurvySplineBase newSpline, out float newTF)
    {
        newSpline = null;
        newTF = 0;
