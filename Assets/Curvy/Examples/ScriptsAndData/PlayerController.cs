...
public class PlayerController : UnityEngine.MonoBehaviour {
    ...
    void Update()
    {
        UnityEngine.Vector3 moveDelta = UnityEngine.Vector3.zero;
        UnityEngine.Vector3 oldPos = mTransform.position; // store old position
        ...
        if (moveaxis != 0) {
            ...
            UnityEngine.Vector3 newPos = Spline.MoveBy(ref TF, ref newdir, UnityEngine.Mathf.Abs(moveaxis) * Speed * Time.smoothDeltaTime, CurvyClamping.Loop);
        ...
        if (jump && mJumpDurationLeft>0) {
                moveDelta += new UnityEngine.Vector3(0, JumpSpeed * Time.smoothDeltaTime, 0);
                mJumpDurationLeft -= Time.deltaTime;
        }
        else  // Gravity (Y--)
            moveDelta += new UnityEngine.Vector3(0, -Gravity * Time.smoothDeltaTime, 0);
        ...
        // The actual moving
        if (moveDelta != UnityEngine.Vector3.zero) {
