...
public class Performance : UnityEngine.MonoBehaviour {
    ...
    void Clear()
    {
        foreach (CurvySpline spl in Splines)
            UnityEngine.GameObject.Destroy(spl.gameObject);
        Splines.Clear();
    }
    ...
    {
#if !UNITY_WEBPLAYER
        string filename = UnityEngine.Application.dataPath + "/CurvyPerformance_" + string.Format("{0:yyyyMMdd_HHmm}", System.DateTime.Now)+".csv";
