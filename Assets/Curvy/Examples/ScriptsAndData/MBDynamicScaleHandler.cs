...
public class MBDynamicScaleHandler : UnityEngine.MonoBehaviour {
    ...
    UnityEngine.Vector3 SplineMesh_OnGetScale(SplinePathMeshBuilder sender, float tf)
    {
        if (insideMesh)  {
            ...
            float d=UnityEngine.Mathf.Abs(delta);
            float s=0;
            if (d<=sphereRadius){
                s = UnityEngine.Mathf.Cos(d / sphereRadius);
            }
            return UnityEngine.Vector3.one + UnityEngine.Vector3.one*s;
        }
