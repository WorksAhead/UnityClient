...
public class CurvySplineGroup : CurvySplineBase
{
    ...
    /// <summary>
    /// Creates a new UnityEngine.GameObject with a CurvySplineGroup
    /// </summary>
    ...
    {
        CurvySplineGroup grp = new UnityEngine.GameObject("Curvy Spline Group", typeof(CurvySplineGroup)).GetComponent<CurvySplineGroup>();
        grp.Add(splines);
        return grp;
    }
    ...
    /// <returns>the interpolated position</returns>
    public override UnityEngine.Vector3 MoveBy(ref float tf, ref int direction, float distance, CurvyClamping clamping, float stepSize)
    {
    ...
    /// <param name="stepSize">stepSize defines the accuracy</param>
    /// <returns>the interpolated position</returns>
    public override UnityEngine.Vector3 MoveByFast(ref float tf, ref int direction, float distance, CurvyClamping clamping, float stepSize)
    {
        return MoveByLengthFast(ref tf, ref direction, distance, clamping);
    }
    ...
    public override UnityEngine.Vector3[] GetApproximation(bool local)
    {
        ...
        List<UnityEngine.Vector3[]> ap = new List<UnityEngine.Vector3[]>(Count);
        
        int n = 0;
        bool con;
        for (int i=0;i<Count;i++) {
            UnityEngine.Vector3[] p = this[i].GetApproximation(local);
            con = NextSplineConnected(i);
            if (con) {
                System.Array.Resize<UnityEngine.Vector3>(ref p,p.Length-1);
            }
            ap.Add(p);
            n += ap[i].Length;
        }

        UnityEngine.Vector3[] apps = new UnityEngine.Vector3[n];
        n = 0;
        for (int i = 0; i < Count; i++) {
            ap[i].CopyTo(apps, n);
            n += ap[i].Length;
    ...
    public override UnityEngine.Vector3[] GetApproximationT()
    {
        ...
        bool con;
        for (int i = 0; i < Count; i++) {
            UnityEngine.Vector3[] p = this[i].GetApproximationT();
            con = NextSplineConnected(i);
            if (con) {
                System.Array.Resize<UnityEngine.Vector3>(ref p, p.Length - 1);
            }
    ...
    public override UnityEngine.Vector3[] GetApproximationUpVectors()
    {
        ...
        bool con;
        for (int i = 0; i < Count; i++) {
            UnityEngine.Vector3[] p = this[i].GetApproximationUpVectors();
            con = NextSplineConnected(i);
            if (con) {
                System.Array.Resize<UnityEngine.Vector3>(ref p, p.Length - 1);
            }
    ...
    {
        UnityEngine.Mathf.Clamp(idx, 0, Count - 1);
        int n = (idx == Count - 1) ? 0 : idx + 1;
        return (idx < Count-1 && UnityEngine.Mathf.Abs((this[idx].InterpolateFast(1) - this[n].InterpolateFast(0)).sqrMagnitude) <= float.Epsilon * float.Epsilon);
    }
