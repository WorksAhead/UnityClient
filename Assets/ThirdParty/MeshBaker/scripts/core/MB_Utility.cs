...
public class MB_Utility{
	...
	public static Material[] GetGOMaterials(UnityEngine.GameObject go){
		...
		if (mr != null){
			...
			if (mf == null){
				throw new Exception("UnityEngine.Object " + go + " has a MeshRenderer but no MeshFilter.");
			}
			mesh = mf.sharedMesh;
		}
		...
		if (sharedMaterials == null){
			Debug.LogError("UnityEngine.Object " + go.name + " does not have a MeshRenderer or a SkinnedMeshRenderer component");
			return null;	
		} else if (mesh == null){
			Debug.LogError("UnityEngine.Object " + go.name + " has a MeshRenderer or SkinnedMeshRenderer but no mesh.");
			return null;				
		} else {
			if (mesh.subMeshCount < sharedMaterials.Length){
				Debug.LogWarning("UnityEngine.Object " + go + " has only " + mesh.subMeshCount + " submeshes and has " + sharedMaterials.Length + " materials. Extra materials do nothing.");	
	...
	public static Mesh GetMesh(UnityEngine.GameObject go){
		if (go == null) return null;
		MeshFilter mf = go.GetComponent<MeshFilter>();
		if (mf != null){
		...
		Debug.LogError("UnityEngine.Object " + go.name + " does not have a MeshFilter or a SkinnedMeshRenderer component");
		return null;
	}
	
	public static UnityEngine.Renderer GetRenderer(UnityEngine.GameObject go){
		if (go == null) return null;
		MeshRenderer mr = go.GetComponent<MeshRenderer>();
	...
	public static void DisableRendererInSource(UnityEngine.GameObject go){
		if (go == null) return;
	...
	public static void setSolidColor(Texture2D t, UnityEngine.Color c){
		UnityEngine.Color[] cs = t.GetPixels();
