...
//						img.y = r.y;
		...
		static void drawGizmosNode(Node r){
			UnityEngine.Vector3 extents = new UnityEngine.Vector3(r.r.w, r.r.h, 0);
			UnityEngine.Vector3 pos = new UnityEngine.Vector3(r.r.x + extents.x/2, -r.r.y - extents.y/2, 0f);
			Gizmos.DrawWireCube(pos,extents);
			if (r.img != null){
				Gizmos.color = UnityEngine.Color.blue;
				extents = new UnityEngine.Vector3(r.img.w, r.img.h, 0);
				pos = new UnityEngine.Vector3(r.img.x + extents.x/2, -r.img.y - extents.y/2, 0f);
		...
		static Texture2D createFilledTex(UnityEngine.Color c, int w, int h){
		...
		bool Probe(Image[] imgsToAdd, int idealAtlasW, int idealAtlasH, float imgArea, int maxAtlasDim, ProbeResult pr){
			...
			for (int i = 0; i < imgsToAdd.Length; i++){
				...
				if (n == null){
					...
					if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Probe success efficiency w=" + usedW + " h=" + usedH + " e=" + efficiency + " sq=" + squareness + " fits=" + fitsInMaxDim);
					return true;
				}
			}	
			if (LOG_LEVEL >= MB2_LogLevel.error) Debug.LogError("Should never get here.");
			return false;
		}
		...
		public Rect[] GetRects(List<UnityEngine.Vector2> imgWidthHeights, int maxDimension, int padding, out int outW, out int outH){
			float area = 0;
			int maxW = 0;
			int maxH = 0;
			...
			for (int i = 0; i < imgsToAdd.Length; i++){
				...
				area += im.w * im.h;
				maxW = UnityEngine.Mathf.Max(maxW, im.w);
				maxH = UnityEngine.Mathf.Max(maxH, im.h);
			}
			
			if ((float)maxH/(float)maxW > 2){
				if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Using height Comparer");
				Array.Sort(imgsToAdd,new ImageHeightComparer());
			}
			else if ((float)maxH/(float)maxW < .5){
				if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Using width Comparer");
				Array.Sort(imgsToAdd,new ImageWidthComparer());
			}
			else{
				if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Using area Comparer");
...
//			List<Node> ns = new List<Node>();
			...
			int sqrtArea = (int) UnityEngine.Mathf.Sqrt(area);
			int idealAtlasW = sqrtArea; 
			int idealAtlasH = sqrtArea; 
			if (maxW > sqrtArea){
				idealAtlasW = maxW;
				idealAtlasH = UnityEngine.Mathf.Max(UnityEngine.Mathf.CeilToInt(area / maxW), maxH);
			}
			if (maxH > sqrtArea){
				idealAtlasW = UnityEngine.Mathf.Max(UnityEngine.Mathf.CeilToInt(area / maxH), maxW);
				idealAtlasH = maxH;
			}
...
//			bool successH = false;
			...
			while (numWIterations > 1 && steppedHeight < sqrtArea * 1000){	
				...
				while (!successW && steppedWidth < sqrtArea * 1000){	
					...
					if (Probe(imgsToAdd, steppedWidth, steppedHeight, area, maxDimension, pr)){
						...
						numWIterations++;
						steppedWidth += stepW;
						if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("increasing Width h=" + steppedHeight + " w=" + steppedWidth);
					}			
				}
				steppedHeight += stepH;
				if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("increasing Height h=" + steppedHeight + " w=" + steppedWidth);
			}
			
			outW = 0;
			outH = 0;
			if (bestRoot == null) return null;
			if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Best fit found: w=" + bestRoot.w + " h=" + bestRoot.h + " efficiency=" + bestRoot.efficiency + " squareness=" + bestRoot.squareness + " fits in max dimension=" + bestRoot.fitsInMaxSize);
			outW = bestRoot.w;
			outH = bestRoot.h;
			...
			for (int i = 0; i < images.Count; i++){
				...
				if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Image: " + i + " imgID=" + im.imgId + " x=" + r.x * outW +
						   " y=" + r.y * outH + " w=" + r.width * outW +
					       " h=" + r.height * outH + " padding=" + padding);
			}
					
			if (LOG_LEVEL >= MB2_LogLevel.info) Debug.Log("Done GetRects");
			return rs;			
		}
...
//			int max = 256;
			...
			List<UnityEngine.Vector2> imgsToAdd = new List<UnityEngine.Vector2>();
//			for (int i = 0; i < numTex; i++){
//				imgsToAdd.Add(new UnityEngine.Vector2(UnityEngine.Random.Range(min,max), UnityEngine.Random.Range(min,max)*5));
//			}
			
			
			
			imgsToAdd.Add(new UnityEngine.Vector2(128,128));
			imgsToAdd.Add(new UnityEngine.Vector2(256,256));
			imgsToAdd.Add(new UnityEngine.Vector2(512,512));
