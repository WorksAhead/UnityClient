...
public class MB2_MultiMeshCombiner {
	...
	static UnityEngine.GameObject[] empty = new UnityEngine.GameObject[0];
	
	Dictionary<UnityEngine.GameObject,CombinedMesh> obj2MeshCombinerMap = new Dictionary<UnityEngine.GameObject, CombinedMesh>();	
	...
	public class CombinedMesh{
		...
		public List<UnityEngine.GameObject> gosToAdd;
		public List<UnityEngine.GameObject> gosToDelete;
		public List<UnityEngine.GameObject> gosToUpdate;
		...
		public CombinedMesh(int maxNumVertsInMesh){
			...
			numVertsInListToAdd = 0;
		 	gosToAdd = new List<UnityEngine.GameObject>();
		 	gosToDelete = new List<UnityEngine.GameObject>();
			gosToUpdate = new List<UnityEngine.GameObject>();
		}
	...
	[SerializeField] UnityEngine.GameObject __resultSceneObject;
	public UnityEngine.GameObject resultSceneObject { 
	...
	public List<UnityEngine.GameObject> GetObjectsInCombined(){ //todo look at getting from keys
		List<UnityEngine.GameObject> allObjs = new List<UnityEngine.GameObject>();
...
*/	
	...
	bool _validate(UnityEngine.GameObject[] gos, UnityEngine.GameObject[] deleteGOs){
		...
		if (gos != null){
			for (int i = 0; i < gos.Length; i++){
				...
				if (obj2MeshCombinerMap.ContainsKey(gos[i])){
					...
					if (!isInDeleteList){
						Debug.LogError("UnityEngine.GameObject " + gos[i] + " is already in the combined mesh");
						return false;
					}
				}
			}
...
//				}				
				...
				if (!obj2MeshCombinerMap.ContainsKey(deleteGOs[i])){
					Debug.LogWarning("UnityEngine.GameObject " + deleteGOs[i] + " on the list of objects to delete is not in the combined mesh.");				
				}				
			}
		}
		return true;
	...
	void _distributeAmongBakers(UnityEngine.GameObject[] gos, UnityEngine.GameObject[] deleteGOs){
		...
		if (resultSceneObject == null) resultSceneObject = new UnityEngine.GameObject("CombinedMesh-" + name);
...
//				lod.SetStatus(MB2_LODStatus.inMesh);				
	...
	public void DestroyMesh(){
		for (int i = 0; i < meshCombiners.Count; i++){
#if UNITY_EDITOR
			if (meshCombiners[i].combinedMesh.targetRenderer != null) UnityEngine.GameObject.DestroyImmediate( meshCombiners[i].combinedMesh.targetRenderer.gameObject );
#else
			if (meshCombiners[i].combinedMesh.targetRenderer != null) UnityEngine.GameObject.Destroy( meshCombiners[i].combinedMesh.targetRenderer.gameObject );
#endif
