...
///		* EasyTouch new
/// 	----------------
/// 	- Add new static method SetCamera(UnityEngine.Camera cam) : To set the current camera use by EasyTouch
...
///		* Gesture class
/// 	-----------------
/// 		- Add method (GetScreenToWordlPoint( UnityEngine.Camera cam,float z) that return the world coordinate position for a camera and z position
...
/// </summary>
public class EasyTouch : UnityEngine.MonoBehaviour {
  ...
  public delegate void SwipeEnd2FingersHandler(EasyTouchGesture gesture);
	...
	public UnityEngine.GameObject receiverObject = null; // Other object that can receive messages.
	...
	public UnityEngine.Camera easyTouchCamera;			// The main camera
	...
	public List<UnityEngine.Camera> nGUICameras = new List<UnityEngine.Camera>();
	...
	// Second Finger
	public UnityEngine.KeyCode twistKey = UnityEngine.KeyCode.LeftAlt;
	public UnityEngine.KeyCode swipeKey = UnityEngine.KeyCode.LeftControl;
	...
	private UnityEngine.GameObject pickObject2Finger;
	private UnityEngine.GameObject oldPickObject2Finger;
	
	
	public UnityEngine.Texture secondFingerTexture;							// The texture to display the simulation of the second finger.
	
	private UnityEngine.Vector2 startPosition2Finger;							// Start position for two fingers gesture
	private int twoFinger0;											// finger index
	private int twoFinger1;											// finger index
	private UnityEngine.Vector2 oldStartPosition2Finger;
	...
	#region UnityEngine.MonoBehaviour methods
	void OnEnable(){
	...
	void InitEasyTouch(){
		...
		if (easyTouchCamera == null){
			easyTouchCamera = UnityEngine.Camera.main;
		...
		// The texture to display the simulation of the second finger.
			...
			if (secondFingerTexture==null){
				secondFingerTexture =Resources.Load("secondFinger") as UnityEngine.Texture;
			}
		#endif		
	}
	...
	#if ((!UNITY_ANDROID && !UNITY_IPHONE) || UNITY_EDITOR)
	void OnGUI(){
		UnityEngine.Vector2 finger = input.GetSecondFingerPosition();
		if (finger!=new UnityEngine.Vector2(-1,-1)){		
	...
	void Update(){
		...
		if (enable && EasyTouch.instance==this){
			...
			if (oldTouchCount==2 && count!=2 && count>0){
				CreateGesture2Finger(EventName.On_Cancel2Fingers,UnityEngine.Vector2.zero,UnityEngine.Vector2.zero,UnityEngine.Vector2.zero,0,SwipeType.None,0,UnityEngine.Vector2.zero,0,0,0);
			}
	...
	private void OneFinger(int fingerIndex){
      ...
      // we notify a touch
      CreateGesture(fingerIndex, EventName.On_TouchStart, fingers[fingerIndex], 0, SwipeType.None, 0, UnityEngine.Vector2.zero);
		}
		...
		if (fingers[fingerIndex].phase != TouchPhase.Ended && fingers[fingerIndex].phase != TouchPhase.Canceled){
			...
			if (fingers[fingerIndex].phase == TouchPhase.Stationary && timeSinceStartAction >= longTapTime && fingers[fingerIndex].gesture == GestureType.Acquisition){
				...
				CreateGesture(fingerIndex, EventName.On_LongTapStart,fingers[fingerIndex],timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero);	
			}
			...
			if ((fingers[fingerIndex].gesture == GestureType.Acquisition ||fingers[fingerIndex].gesture == GestureType.LongTap) && (FingerInTolerance(fingers[fingerIndex])==false) ){
				...
				if (fingers[fingerIndex].gesture == GestureType.LongTap){
					fingers[fingerIndex].gesture = GestureType.Cancel;
					CreateGesture(fingerIndex, EventName.On_LongTapEnd,fingers[fingerIndex],timeSinceStartAction,SwipeType.None,0,UnityEngine.Vector2.zero);
					...
					if (fingers[fingerIndex].pickedObject){
						...
						CreateGesture(fingerIndex, EventName.On_DragStart,fingers[fingerIndex],timeSinceStartAction,SwipeType.None,0, UnityEngine.Vector2.zero);
					}
					// If not swipe
					else{
						fingers[fingerIndex].gesture = GestureType.Swipe;
						CreateGesture(fingerIndex, EventName.On_SwipeStart,fingers[fingerIndex],timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero);
					}
				}
			}
      ...
      if (message != EventName.None) {
        currentSwipe = GetSwipe(new UnityEngine.Vector2(0, 0), fingers[fingerIndex].deltaPosition);
      ...
      CreateGesture(fingerIndex, EventName.On_TouchDown, fingers[fingerIndex], timeSinceStartAction, currentSwipe, 0, fingers[fingerIndex].deltaPosition);
			...
			switch (fingers[fingerIndex].gesture){
				...
				case GestureType.Acquisition:
					if (FingerInTolerance(fingers[fingerIndex])){
						if (fingers[fingerIndex].tapCount<2){
							CreateGesture( fingerIndex, EventName.On_SimpleTap,fingers[fingerIndex], timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero);
						}
						else{
							CreateGesture( fingerIndex, EventName.On_DoubleTap,fingers[fingerIndex], timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero);
						}
					
					}
					else{
						SwipeType currentSwipe = GetSwipe(new UnityEngine.Vector2(0,0),fingers[fingerIndex].deltaPosition);
						if (fingers[fingerIndex].pickedObject){
							CreateGesture(fingerIndex, EventName.On_DragStart,fingers[fingerIndex],timeSinceStartAction,SwipeType.None,0, UnityEngine.Vector2.zero);
				...
				case GestureType.Cancel:
					CreateGesture(fingerIndex, EventName.On_Cancel,fingers[fingerIndex],0,SwipeType.None,0,UnityEngine.Vector2.zero);
					break;
			}
				
			if (realEnd){
				CreateGesture( fingerIndex, EventName.On_TouchUp,fingers[fingerIndex], timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero);
	...
	private void CreateGesture(int touchIndex,EventName message,Finger finger,float actionTime, SwipeType swipe, float swipeLength, UnityEngine.Vector2 swipeVector){
  ...
  private void SendGesture(EventName message, EasyTouchGesture gesture)
  {
		...
		if (useBroadcastMessage){
			// Sent to user UnityEngine.GameObject
			...
			// Sent to the  UnityEngine.GameObject who is selected
	...
	private void TwoFinger(){
		...
		UnityEngine.Vector2 deltaPosition = UnityEngine.Vector2.zero;
		...
		if ( complexCurrentGesture==GestureType.None){
			...
			oldFingerDistance = UnityEngine.Mathf.Abs( UnityEngine.Vector2.Distance(fingers[twoFinger0].position, fingers[twoFinger1].position));
			startPosition2Finger = new UnityEngine.Vector2((fingers[twoFinger0].position.x+fingers[twoFinger1].position.x)/2, (fingers[twoFinger0].position.y+fingers[twoFinger1].position.y)/2);
			deltaPosition = UnityEngine.Vector2.zero;
			...
			// we notify the touch
			CreateGesture2Finger(EventName.On_TouchStart2Fingers,startPosition2Finger,startPosition2Finger,deltaPosition,timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,oldFingerDistance);				
		}
		...
		position = new  UnityEngine.Vector2((fingers[twoFinger0].position.x+fingers[twoFinger1].position.x)/2, (fingers[twoFinger0].position.y+fingers[twoFinger1].position.y)/2);
		deltaPosition = position - oldStartPosition2Finger;
		fingerDistance = UnityEngine.Mathf.Abs(UnityEngine.Vector2.Distance(fingers[twoFinger0].position, fingers[twoFinger1].position));
		...
		if (fingers[twoFinger0].phase != TouchPhase.Ended && fingers[twoFinger1].phase != TouchPhase.Ended && complexCurrentGesture != GestureType.Cancel ){
			...
			if (complexCurrentGesture == GestureType.Tap && timeSinceStartAction >= longTapTime && FingerInTolerance(fingers[twoFinger0]) && FingerInTolerance(fingers[twoFinger1])){	
				...
				CreateGesture2Finger(EventName.On_LongTapStart2Fingers,startPosition2Finger,position,deltaPosition,timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,fingerDistance);				
			}	
			...
			if (move){
				...
				float dot = UnityEngine.Vector2.Dot(fingers[twoFinger0].deltaPosition.normalized, fingers[twoFinger1].deltaPosition.normalized);
																																															
				// Pinch
				if (enablePinch && fingerDistance != oldFingerDistance ){
					// Pinch
					if (UnityEngine.Mathf.Abs( fingerDistance-oldFingerDistance)>=minPinchLength){
					...
					if (complexCurrentGesture == GestureType.Pinch){	
						...
						if (fingerDistance<oldFingerDistance){
							...
							// Send pinch
							CreateGesture2Finger(EventName.On_PinchIn,startPosition2Finger,position,deltaPosition,timeSinceStartAction, GetSwipe(fingers[twoFinger0].complexStartPosition,fingers[twoFinger0].position),0,UnityEngine.Vector2.zero,0,UnityEngine.Mathf.Abs(fingerDistance-oldFingerDistance),fingerDistance);
							complexCurrentGesture = GestureType.Pinch;
							...
							// Send pinch
							CreateGesture2Finger(EventName.On_PinchOut,startPosition2Finger,position,deltaPosition,timeSinceStartAction, GetSwipe(fingers[twoFinger0].complexStartPosition,fingers[twoFinger0].position),0,UnityEngine.Vector2.zero,0,UnityEngine.Mathf.Abs(fingerDistance-oldFingerDistance),fingerDistance);
							complexCurrentGesture = GestureType.Pinch;
						}	
					}
				}
				...
				if (enableTwist){
					...
					if (UnityEngine.Mathf.Abs(TwistAngle())>minTwistAngle){
					...
					// Update Twist
					if (complexCurrentGesture == GestureType.Twist){
						CreateGesture2Finger(EventName.On_Twist,startPosition2Finger,position,deltaPosition,timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero,TwistAngle(),0,fingerDistance);
					}
				...
				if (dot>0 ){
					if (pickObject2Finger && !twoFingerDragStart){
						...
						//
						CreateGesture2Finger(EventName.On_DragStart2Fingers,startPosition2Finger,position,deltaPosition,timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,fingerDistance);	
						...
						CreateGesture2Finger(EventName.On_SwipeStart2Fingers,startPosition2Finger,position,deltaPosition,timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,fingerDistance);
				...
				// Long tap update
		 		if (complexCurrentGesture == GestureType.LongTap){
					CreateGesture2Finger(EventName.On_LongTap2Fingers,startPosition2Finger,position,deltaPosition,timeSinceStartAction, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,fingerDistance);
				}
			}
	...
	private void CreateStateEnd2Fingers(GestureType gesture, UnityEngine.Vector2 startPosition, UnityEngine.Vector2 position, UnityEngine.Vector2 deltaPosition,float time, bool realEnd,float fingerDistance){
		...
		switch (gesture){
			...
			case GestureType.Tap:
				...
				if (fingers[twoFinger0].tapCount<2 && fingers[twoFinger1].tapCount<2){
					...
					time, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,fingerDistance);				
				}
				else{
		...
		if (realEnd){
			...
			CreateGesture2Finger(EventName.On_TouchUp2Fingers,startPosition,position,deltaPosition,time, SwipeType.None,0,UnityEngine.Vector2.zero,0,0,fingerDistance);
		}
	}

	private void  CreateGesture2Finger(EventName message,UnityEngine.Vector2 startPosition,UnityEngine.Vector2 position,UnityEngine.Vector2 deltaPosition,
	float actionTime, SwipeType swipe, float swipeLength,UnityEngine.Vector2 swipeVector,float twist,float pinch, float twoDistance){
  ...
  private void RaiseEvent(EventName evnt, EasyTouchGesture gesture)
  {
	...
	private UnityEngine.GameObject GetPickeGameObject(UnityEngine.Vector2 screenPos){
	...
	private SwipeType GetSwipe(UnityEngine.Vector2 start, UnityEngine.Vector2 end){
	
		UnityEngine.Vector2 linear;
		linear = (end - start).normalized;
		
		if (UnityEngine.Mathf.Abs(linear.y)>UnityEngine.Mathf.Abs(linear.x)){
			if ( UnityEngine.Vector2.Dot( linear, UnityEngine.Vector2.up) >= swipeTolerance)
				return SwipeType.Up;
			...
			if ( UnityEngine.Vector2.Dot( linear, UnityEngine.Vector2.right) >= swipeTolerance)
	...
	private float DeltaAngle(UnityEngine.Vector2 start, UnityEngine.Vector2 end){
	
		var tmp = (start.x * end.y)-(start.y*end.x);
		return UnityEngine.Mathf.Atan2(tmp,UnityEngine.Vector2.Dot( start,end));
		
	}

	private float TwistAngle(){
	
		UnityEngine.Vector2 dir = (fingers[twoFinger0].position-fingers[twoFinger1].position);
		UnityEngine.Vector2 refDir =(fingers[twoFinger0].oldPosition - fingers[twoFinger1].oldPosition);
		float angle =  UnityEngine.Mathf.Rad2Deg * DeltaAngle(refDir,dir);
	...
	/// The camera
	/// </param>
	public static void SetCamera(UnityEngine.Camera cam){
	...
	/// </param>
	public static void SetOtherReceiverObject( UnityEngine.GameObject receiver){
	...
	/// UnityEngine.GameObject
	/// </returns>
	public static UnityEngine.GameObject GetOtherReceiverObject(){
	...
	/// Finger index.
	/// </param>
	public static UnityEngine.GameObject GetCurrentPickedObject(int fingerIndex){
	...
	/// Finger index.
	/// </param>
	public static UnityEngine.Vector2 GetFingerPosition(int fingerIndex){
