...
/// </summary>
[ExecuteInEditMode]
public class SplinePathCloneBuilder : UnityEngine.MonoBehaviour
{
    ...
    public UnityEngine.GameObject[] Source = new UnityEngine.GameObject[0];
    /// <summary>
    ...
    public delegate UnityEngine.GameObject OnGetCloneEvent(SplinePathCloneBuilder sender, UnityEngine.GameObject source);
    public delegate void OnReleaseCloneEvent(SplinePathCloneBuilder sender, UnityEngine.GameObject clone);
    /// <summary>
    ...
    /// <summary>
    /// Creates a UnityEngine.GameObject with a SplinePathCloneBuilder script attached
    /// </summary>
    public static SplinePathCloneBuilder Create()
    {
        SplinePathCloneBuilder o = new UnityEngine.GameObject("CurvyClonePath", typeof(SplinePathCloneBuilder)).GetComponent<SplinePathCloneBuilder>();
        return o;
    }
    ...
    public void Refresh(bool force)
    {
        ...
        int count=0;
        if (!UnityEngine.Mathf.Approximately(0, totaldepth)) {
            switch (Mode) {
                case SplinePathCloneBuilderMode.CloneGroup:
                    count = UnityEngine.Mathf.FloorToInt(Spline.Length / totaldepth) * Source.Length;
                    break;
                default: // Individual
        ...
        if (count >= MAXCLONES) {
            ...
            while (++current < count) {
                ...
                if (current < existing) {
                    UnityEngine.Transform T = mTransform.GetChild(current);
                    if (UseWorldPosition)
                    ...
                    if (clone) {
                        UnityEngine.Transform T = clone.transform;
                        T.parent = transform;
    ...
    public void Clear()
    {
        UnityEngine.Transform[] childs = transform.GetComponentsInChildren<UnityEngine.Transform>();
    ...
    /// <summary>
    /// Parent existing clones to an empty UnityEngine.GameObject, making them persistent
    /// </summary>
    /// <returns>the new UnityEngine.Transform</returns>
    public UnityEngine.Transform Detach()
    {
        var T = new UnityEngine.GameObject().transform;
    ...
    /// </summary>
    /// <returns></returns>
    public void Detach(UnityEngine.Transform to)
    {
        UnityEngine.Transform[] childs = transform.GetComponentsInChildren<UnityEngine.Transform>();
        for (int i = childs.Length - 1; i > 0; i--)
            childs[i].parent = to;
    ...
    UnityEngine.GameObject CloneObject(UnityEngine.GameObject source)
    {
        return (source != null) ? UnityEngine.GameObject.Instantiate(source) as UnityEngine.GameObject : null;
    }

    void DestroyObject(UnityEngine.GameObject obj)
    {
    ...
    void checkSources()
    {
        ...
        Source = (src.Count == 0) ? new UnityEngine.GameObject[0] : (UnityEngine.GameObject[])src.ToArray(typeof(UnityEngine.GameObject));
    }

    // returns z-depth of a collider or 0 on error
    float getDepth(UnityEngine.GameObject o)
    {
        if (!o) return 0;
        // Use all colliders we can find
        Bounds bounds=new Bounds(o.transform.position,UnityEngine.Vector3.zero);
        UnityEngine.Collider[] cols = o.GetComponentsInChildren<UnityEngine.Collider>(true);
