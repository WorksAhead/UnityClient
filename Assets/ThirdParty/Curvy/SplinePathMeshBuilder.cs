...
[ExecuteInEditMode]
public class SplinePathMeshBuilder : UnityEngine.MonoBehaviour
{
    ...
    public delegate UnityEngine.Vector3 OnGetScaleEvent(SplinePathMeshBuilder sender, float tf);
    ...
    /// <summary>
    /// Animation curve to use when ScaleModifier==MeshScaleModifier.UnityEngine.AnimationCurve
    /// </summary>
    public UnityEngine.AnimationCurve ScaleModifierCurve;
    // General
    /// <summary>
    ...
    // resulting mesh data
    UnityEngine.Vector3[] mVerts;
    UnityEngine.Vector2[] mUV;
    int[] mTris;
    float mLastRefresh;
    ...
    /// <summary>
    /// Creates a UnityEngine.GameObject with a SplinePathMeshBuilder script attached
    /// </summary>
    /// <returns></returns>
    public static SplinePathMeshBuilder Create()
    {
        SplinePathMeshBuilder o = new UnityEngine.GameObject("CurvyMeshPath", typeof(SplinePathMeshBuilder)).GetComponent<SplinePathMeshBuilder>();
        return o;
    }

    /// <summary>
    /// Create a new UnityEngine.GameObject containing the mesh
    /// </summary>
    ...
    public void Refresh()
    {
        ...
        if (StartMesh && StartMeshInfo!=null && ToTF - FromTF != 0) {
            mVerts = new UnityEngine.Vector3[getTotalVertexCount()];
            mUV = new UnityEngine.Vector2[mVerts.Length];
    ...
    void Prepare()
    {
        ...
        if (Spline && StartMesh && ExtrusionParameter > 0) {
            ...
            float tf = FromTF;
            mSegmentInfo.Clear();
            FromTF = UnityEngine.Mathf.Clamp01(FromTF);
            ToTF = UnityEngine.Mathf.Max(FromTF, UnityEngine.Mathf.Clamp01(ToTF));
            ...
            if (FromTF != ToTF) {
                ...
                if (!UnityEngine.Mathf.Approximately(tf, ToTF))
                    tf = ToTF;
                scale = getScale(tf);
                mSegmentInfo.Add(new CurvyMeshSegmentInfo(this, tf, scale));
            }
    ...
    int getTotalTriIndexCount()
    {
        ...
        if (StartMesh) {
            ...
            if (StartMeshInfo.LoopVertexCount == 0)
                t += (SegmentSteps - 1) * UnityEngine.Mathf.Max(2,(StartMeshInfo.VertexCount-1)) *2 * 3;
            else
        ...
        if (StartMeshInfo.LoopVertexCount == 0) {
            ...
            for (int seg = 0; seg < segSteps; seg++) { // each segment
                ...
                for (int v = 0; v < StartMeshInfo.VertexCount; v++) {
                    ...
                    mUV[vIndex + v] = new UnityEngine.Vector2(StartMeshInfo.UVs[v].x, vcoord);
                }
            ...
            for (int seg = 0; seg < segSteps; seg++) { // each segment
                ...
                for (int l = 0; l < StartMeshInfo.EdgeLoops.Length; l++) { // each EdgeLoop
                    ...
                    for (int ev = 0; ev < E.vertexCount; ev++) { // each EdgeLoop vertex
                        ...
                        mUV[vIndex + ev] = new UnityEngine.Vector2(StartMeshInfo.EdgeUV[E.vertexIndex[ev]].x, vcoord);
                    }
