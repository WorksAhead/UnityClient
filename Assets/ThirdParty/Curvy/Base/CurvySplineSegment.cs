...
/// </summary>
[ExecuteInEditMode]
public class CurvySplineSegment : UnityEngine.MonoBehaviour
{
    ...
    public UnityEngine.Vector3[] UserValues = new UnityEngine.Vector3[0];
    ...
    [HideInInspector]
    public UnityEngine.Vector3[] Approximation=new UnityEngine.Vector3[0];
    ...
    /// <remarks>Based on a spline's Granularity</remarks>
    [HideInInspector]
    public UnityEngine.Vector3[] ApproximationUp = new UnityEngine.Vector3[0];
    ...
    /// <remarks>Based on a spline's Granularity</remarks>
    [HideInInspector]
    public UnityEngine.Vector3[] ApproximationT = new UnityEngine.Vector3[0];

    /// <summary>
    /// Left B-Spline Handle in local coordinates
    /// </summary>
    public UnityEngine.Vector3 HandleIn=new UnityEngine.Vector3(-1,0,0);
    /// <summary>
    /// Right B-Spline Handle in local coordinates
    /// </summary>
    public UnityEngine.Vector3 HandleOut = new UnityEngine.Vector3(1, 0, 0);
    /// <summary>
    ...
    /// </summary>
    public UnityEngine.Vector3 HandleInPosition
    {
        get
        {
    ...
    /// </summary>
    public UnityEngine.Vector3 HandleOutPosition
    {
        get
        {
    ...
    /// <summary>
    /// Gets the next UnityEngine.Transform
    /// </summary>
    /// <returns>a UnityEngine.Transform or Null</returns>
    public UnityEngine.Transform NextTransform
    {
        get
        {
    ...
    /// <summary>
    /// Gets the previous UnityEngine.Transform
    /// </summary>
    /// <returns>a UnityEngine.Transform or Null</returns>
    public UnityEngine.Transform PreviousTransform
    {
        get
        {
    ...
    UnityEngine.Vector3 mPosition;
    UnityEngine.Quaternion mRotation;
    CurvySpline mSpline;
    float mStepSize;
    ...
    void DoGizmos(bool selected)
    {
        ...
        if (con!=null) {
            switch (con.Sync) {
                case CurvyConnection.SyncMode.NoSync: Gizmos.color = new UnityEngine.Color(0,0,0,0.6f); break;
                case CurvyConnection.SyncMode.SyncPosAndRot: Gizmos.color = new UnityEngine.Color(1,1,1,0.6f); break;
                case CurvyConnection.SyncMode.SyncRot: Gizmos.color = new UnityEngine.Color(1,1,0,0.6f); break;
        ...
        for (float f = 0.05f; f < 1; f += 0.05f) {
            UnityEngine.Vector3 p1 = Interpolate(f);
        ...
        if (Spline.ShowApproximation && Approximation.Length>0) {
            ...
            foreach (UnityEngine.Vector3 pa in Approximation)
                Gizmos.DrawCube(pa, new UnityEngine.Vector3(0.1f, 0.1f, 0.1f));
            UnityEngine.Vector3 p3=Approximation[0];
            for (int i=1;i<Approximation.Length;i++){
                UnityEngine.Vector3 p1 = Approximation[i];
        ...
        if (Spline.ShowTangents && ApproximationT.Length > 0) {
            Gizmos.color = new UnityEngine.Color(0, 0.7f, 0);
    ...
    public UnityEngine.Vector3 Interpolate(float localF) { return Interpolate(localF, Spline.Interpolation); }
    /// <summary>
    /// Interpolates position for a local F
    /// </summary>
    /// <param name="localF">a local F in the range 0..1</param>
    ...
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 Interpolate(float localF, CurvyInterpolation interpolation)
    {
        localF = UnityEngine.Mathf.Clamp01(localF);
        switch (interpolation) {
            ...
            case CurvyInterpolation.CatmulRom:
                    //return CurvySpline.CatmulRom(PreviousTransform.position, UnityEngine.Transform.position, NextTransform.position, NextControlPoint.NextTransform.position, localF);
            ...
            default: // LINEAR
                    return UnityEngine.Vector3.Lerp(Transform.position, NextTransform.position, localF);
        }
    }
    ...
    /// </summary>
    UnityEngine.Vector3 GetPrevPosition()
    {
    ...
    /// </summary>
    /// <returns></returns>
    public UnityEngine.Vector3 GetNextPosition()
    {
    ...
    /// </summary>
    /// <returns></returns>
    UnityEngine.Vector3 GetNextNextPosition()
    {
        var cp = NextControlPoint;
        if (cp) 
    ...
    /// <param name="localF">a local F in the range 0..1</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 InterpolateFast(float localF)
    {
        float frag;
        int idx = getApproximationIndex(localF, out frag);
        return (UnityEngine.Vector3.Lerp(Approximation[idx],Approximation[idx+1],frag));
    }
    ...
    public UnityEngine.Vector3 InterpolateUserValue(float localF, int index)
    {
        if (index >= Spline.UserValueSize || NextControlPoint==null)
            return UnityEngine.Vector3.zero;

        return UnityEngine.Vector3.Lerp(UserValues[index], NextControlPoint.UserValues[index], localF);
    }
    ...
    /// <param name="localF">a local F in the range 0..1</param>
    /// <returns>the interpolated value</returns>
    public UnityEngine.Vector3 InterpolateScale(float localF)
    {
        UnityEngine.Transform T = NextTransform;
        return (T) ? UnityEngine.Vector3.Lerp(Transform.lossyScale, T.lossyScale, localF) : Transform.lossyScale;
    }
    ...
    public UnityEngine.Vector3 GetTangent(float localF)
    {
        localF = UnityEngine.Mathf.Clamp01(localF);
        UnityEngine.Vector3 p = Interpolate(localF);
    ...
    /// <returns></returns>
    public UnityEngine.Vector3 GetTangent(float localF, ref UnityEngine.Vector3 position)
    {
    ...
    /// <param name="localF">a local F in the range 0..1</param>
    /// <returns></returns>
    public UnityEngine.Vector3 GetTangentFast(float localF)
    {
        float frag;
        int idx = getApproximationIndex(localF, out frag);
        return (UnityEngine.Vector3.Lerp(ApproximationT[idx], ApproximationT[idx + 1], frag));
    }
    ...
    public UnityEngine.Quaternion GetOrientationFast(float localF)
    {
        UnityEngine.Vector3 view = GetTangentFast(localF);
        
        if (view != UnityEngine.Vector3.zero)
            return UnityEngine.Quaternion.LookRotation(GetTangentFast(localF), GetOrientationUpFast(localF));
        else
    ...
    public UnityEngine.Vector3 GetOrientationUpFast(float localF)
    {
        float frag;
        int idx = getApproximationIndex(localF, out frag);
        return (UnityEngine.Vector3.Lerp(ApproximationUp[idx], ApproximationUp[idx + 1], frag));
    }
    ...
    {
        localDistance = UnityEngine.Mathf.Clamp(localDistance, 0, Length);
        if (ApproximationDistances.Length == 0 || localDistance==0) return 0;
        if (UnityEngine.Mathf.Approximately(localDistance,Length)) return 1;
    ...
    {
        if (stepSize == 0 || UnityEngine.Mathf.Approximately(newSplineLength, Spline.Length)) return true;

        float curLength = Spline.Length;
        UnityEngine.Vector3 oldPos = Transform.position;
        UnityEngine.Vector3 upstep = Transform.up * stepSize;
    ...
    UnityEngine.Vector3 HandleInScaled
    {
        get
        {
    ...
    UnityEngine.Vector3 HandleOutScaled
    {
        get
        {
    ...
    public void _InitializeControlPoint()
    {
        ...
        Approximation = new UnityEngine.Vector3[0];
        ApproximationDistances = new float[0];
        ApproximationUp = new UnityEngine.Vector3[0];
        ApproximationT = new UnityEngine.Vector3[0];
        if (UserValues.Length != Spline.UserValueSize) 
            System.Array.Resize<UnityEngine.Vector3>(ref UserValues, Spline.UserValueSize);
    }
    ...
    public void _UpdateApproximation()
    {
        ...
        if (IsValidSegment) {
            ...
            mStepSize = 1f / Spline.Granularity;
            Approximation = new UnityEngine.Vector3[Spline.Granularity + 1];
            ApproximationUp = new UnityEngine.Vector3[Spline.Granularity + 1];
            ApproximationT = new UnityEngine.Vector3[Spline.Granularity + 1];
    ...
    public void _UpdateTangents()
    {
        if (IsValidSegment) {
            ...
            if (SmoothEdgeTangent) {
                if (!IsFirstSegment) {
                    PreviousSegment.ApproximationT[Spline.Granularity] = UnityEngine.Vector3.Lerp(PreviousSegment.ApproximationT[Spline.Granularity - 1], ApproximationT[0], 0.5f);
            ...
            if (IsLastSegment && NextSegment && Spline[0].SmoothEdgeTangent) { // closed spline
                ApproximationT[Spline.Granularity] = UnityEngine.Vector3.Lerp(ApproximationT[Spline.Granularity - 1], Spline[0].ApproximationT[0], 0.5f);
                Spline[0].ApproximationT[0] = ApproximationT[Spline.Granularity];
            }
        }
    }

    UnityEngine.Vector3 ParallelTransportFrame(ref UnityEngine.Vector3 lastUp, ref UnityEngine.Vector3 T1, ref UnityEngine.Vector3 T2, float swirlangle)
    {
        UnityEngine.Vector3 A = UnityEngine.Vector3.Cross(T1, T2);
        float a = UnityEngine.Mathf.Atan2(A.magnitude, UnityEngine.Vector3.Dot(T1, T2));
        UnityEngine.Quaternion Q = UnityEngine.Quaternion.AngleAxis(UnityEngine.Mathf.Rad2Deg * a, A.normalized);
        if (Spline.Swirl!=CurvyOrientationSwirl.None)
            return Q * UnityEngine.Quaternion.AngleAxis(swirlangle,T2) * lastUp;
        else
            return Q * lastUp;
    }

    // returns the last T
    public UnityEngine.Vector3 _UpdateOrientation(UnityEngine.Vector3 lastUpVector)
    {
        // Setup swirling
        float swirlangle=0;
        switch (Spline.Swirl) {
        ...
        if (Spline.Orientation == CurvyOrientation.Tangent) {
            ...
            int al = Approximation.Length;
            for (i = 0; i < al; i++) {
                ApproximationUp[i] = UnityEngine.Vector3.Lerp(Transform.up, NextTransform.up, i * mStepSize);
            }
    ...
    // returns the last T
    public UnityEngine.Vector3 _SmoothOrientation(UnityEngine.Vector3 lastUpVector,ref float angleaccu, float angle)
    {
        ApproximationUp[0] = lastUpVector;
        for (int i = 1; i < ApproximationUp.Length; i++) {
            ApproximationUp[i] = UnityEngine.Quaternion.AngleAxis(angleaccu, ApproximationT[i]) * ApproximationUp[i];
            angleaccu += angle;
        }
