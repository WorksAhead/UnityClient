...
public class CurvySpline : CurvySplineBase
{
    ...
    /// <summary>
    /// If set, Control Point's rotation will be set to the calculated Up-UnityEngine.Vector3
    /// </summary>
    ...
    /// <summary>
    /// UnityEngine.Color used by spline gizmo
    /// </summary>
    public static UnityEngine.Color GizmoColor = UnityEngine.Color.red;
    /// <summary>
    /// UnityEngine.Color used by spline gizmo for selected Control Points / Segments
    /// </summary>
    public static UnityEngine.Color GizmoSelectionColor = UnityEngine.Color.white;
    /// <summary>
    ...
    {
        CurvySpline spl = new UnityEngine.GameObject("Curvy Spline", typeof(CurvySpline)).GetComponent<CurvySpline>();
        return spl;
    }
    ...
    void Update()
    {
        if (UnityEngine.Application.isPlaying && !IsInitialized) {
            RefreshImmediately(AutoRefreshLength, AutoRefreshOrientation, false);
        
        }
        if (!UnityEngine.Application.isPlaying || AutoRefresh) {
            bool refresh = false;
    ...
    public override UnityEngine.Vector3 Interpolate(float tf)
    {
    ...
    /// <returns>the interpolated position</returns>
    public override UnityEngine.Vector3 Interpolate(float tf, CurvyInterpolation interpolation) 
    {
        float localF;
    ...
    public override UnityEngine.Vector3 InterpolateUserValue(float tf, int index)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
    ...
    /// <param name="tf">TF value reflecting position on spline(0..1)</param>
    /// <returns>the interpolated value</returns>
    public override UnityEngine.Vector3 InterpolateScale(float tf)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
    ...
    public override UnityEngine.Vector3 GetOrientationUpFast(float tf)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
    ...
    /// <returns>a tangent vector</returns>
    public override UnityEngine.Vector3 GetTangent(float tf, UnityEngine.Vector3 position)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
    ...
    /// <param name="tf">TF value identifying position on spline (0..1)</param>
    /// <returns>a tangent vector</returns>
    public override UnityEngine.Vector3 GetTangentFast(float tf)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
    ...
    {
        tf = UnityEngine.Mathf.Clamp01(tf);
        localF = 0;
    ...
    {
        distance = UnityEngine.Mathf.Clamp(distance,0, Length);
        localDistance = 0;
    ...
    public List<CurvyConnection> GetConnectionsWithin(float tf, int direction, float fDistance, int minMatchesNeeded, bool skipCurrent, params string[] tags)
    {
        ...
        if (fdelta >= 0) {
            ...
            if (fromLocalF > 0) // don't check a CP we already passed
                fromIdx++;
            if (toLocalF == 1)
                toIdx = UnityEngine.Mathf.Min(ControlPointCount - 1, toIdx + 1);
    ...
    /// <param name="tags">list of tags to match</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 MoveConnection(ref CurvySpline spline, ref float tf, ref int direction, float fDistance, CurvyClamping clamping, int minMatchesNeeded, params string[] tags)
    {
    ...
    /// <param name="tags">list of tags to match</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 MoveConnectionFast(ref CurvySpline spline, ref float tf, ref int direction, float fDistance, CurvyClamping clamping, int minMatchesNeeded, params string[] tags)
    {
        List<CurvyConnection> cons = GetConnectionsWithin(tf, direction, fDistance, minMatchesNeeded,true, tags);
        if (cons.Count > 0) {
            CurvyConnection con;
    ...
    /// <param name="tags">list of tags to match</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 MoveByConnection(ref CurvySpline spline, ref float tf, ref int direction, float distance, CurvyClamping clamping, int minMatchesNeeded, params string[] tags)
    {
    ...
    /// <param name="tags">list of tags to match</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 MoveByConnection(ref CurvySpline spline, ref float tf, ref int direction, float distance, CurvyClamping clamping, int minMatchesNeeded, float stepSize, params string[] tags)
    {
    ...
    /// <param name="tags">list of tags to match</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 MoveByConnectionFast(ref CurvySpline spline, ref float tf, ref int direction, float distance, CurvyClamping clamping, int minMatchesNeeded, params string[] tags)
    {
    ...
    /// <param name="tags">list of tags to match</param>
    /// <returns>the interpolated position</returns>
    public UnityEngine.Vector3 MoveByConnectionFast(ref CurvySpline spline, ref float tf, ref int direction, float distance, CurvyClamping clamping, int minMatchesNeeded, float stepSize, params string[] tags)
    {
    ...
    public CurvySplineSegment[] Add(params UnityEngine.Vector3[] controlPoints)
    {
    ...
    {
        UnityEngine.GameObject go = new UnityEngine.GameObject("NewCP", typeof(CurvySplineSegment));
        ...
        if (insertAfter) {
            if (insertAfter.IsValidSegment)
                ...
                go.transform.position = UnityEngine.Vector3.Lerp(insertAfter.NextTransform.position, insertAfter.Transform.position, 0.5f);
        ...
        if (insertBefore) {
            if (insertBefore.PreviousSegment)
                ...
                if (insertBefore.PreviousTransform)
                    go.transform.position = UnityEngine.Vector3.Lerp(insertBefore.PreviousTransform.position, insertBefore.Transform.position, 0.5f);
            
            idx = UnityEngine.Mathf.Max(0,insertBefore.ControlPointIndex);
        }
    ...
    public override UnityEngine.Vector3[] GetApproximation() { return GetApproximation(false); }
    /// <summary>
    /// Gets an array containing all approximation points
    /// </summary>
    ...
    public override UnityEngine.Vector3[] GetApproximation(bool local)
    {
        UnityEngine.Vector3[] apps = new UnityEngine.Vector3[Count * Granularity+1];
        int idx = 0;
        for (int si = 0; si < Count; si++) {
            this[si].Approximation.CopyTo(apps, idx);
            idx += UnityEngine.Mathf.Max(0,this[si].Approximation.Length-1);
        }
    ...
    public override UnityEngine.Vector3[] GetApproximationT()
    {
        UnityEngine.Vector3[] apps = new UnityEngine.Vector3[Count * Granularity+1];
        int idx = 0;
        for (int si = 0; si < Count; si++) {
            this[si].ApproximationT.CopyTo(apps, idx);
            idx += UnityEngine.Mathf.Max(0, this[si].ApproximationT.Length - 1);
        }
        return apps;
    }
    ...
    public override UnityEngine.Vector3[] GetApproximationUpVectors()
    {
        UnityEngine.Vector3[] apps = new UnityEngine.Vector3[Count * Granularity+1];
        int idx = 0;
        for (int si = 0; si < Count; si++) {
            this[si].ApproximationUp.CopyTo(apps, idx);
            idx += UnityEngine.Mathf.Max(0, this[si].ApproximationUp.Length - 1);
        }
        return apps;
    }
    ...
    public override float GetNearestPointTF(UnityEngine.Vector3 p)
    {
    ...
    /// <remarks>The speed as well as the accuracy depends on the Granularity</remarks>
    /// <returns>a TF value in the range 0..1 or -1 on error</returns>
    public float GetNearestPointTFExt(UnityEngine.Vector3 p, params CurvySplineSegment[] segmentsToCheck)
    {
    ...
    /// <summary>
    /// Gets the next UnityEngine.Transform of a certain Control Point
    /// </summary>
    /// <param name="controlpoint">a Control Point </param>
    /// <returns>a UnityEngine.Transform or Null</returns>
    public UnityEngine.Transform NextTransform(CurvySplineSegment controlpoint)
    {
    ...
    /// <summary>
    /// Gets the previous UnityEngine.Transform of a certain Control Point
    /// </summary>
    /// <param name="controlpoint">a Control Point </param>
    /// <returns>a UnityEngine.Transform or Null</returns>
    public UnityEngine.Transform PreviousTransform(CurvySplineSegment controlpoint)
    {
    ...
    public override void RefreshImmediately(bool refreshLength, bool refreshOrientation, bool skipIfInitialized)
    {
        ...
        UserValueSize = UnityEngine.Mathf.Max(0, UserValueSize);
        mSegments.Clear();
        ...
        if (refreshOrientation) {
            ...
            if (!UnityEngine.Application.isPlaying && ControlPointCount>0 && ControlPoints[ControlPointCount - 1].ConnectedBy.Count > 0) {
    ...
    public UnityEngine.Vector3 GetExtrusionPoint(UnityEngine.Vector3 P, UnityEngine.Vector3 Tangent, UnityEngine.Vector3 Up, float radius, float angle)
    {
        UnityEngine.Quaternion R = UnityEngine.Quaternion.AngleAxis(angle, Tangent);
    ...
    /// <returns></returns>
    public static UnityEngine.Vector3 Bezier(UnityEngine.Vector3 T0, UnityEngine.Vector3 P0, UnityEngine.Vector3 P1, UnityEngine.Vector3 T1, float f)
    {
        ...
        return new UnityEngine.Vector3(FX, FY, FZ);
    }
    ...
    /// <returns>the interpolated position</returns>
    public static UnityEngine.Vector3 TCB(UnityEngine.Vector3 T0, UnityEngine.Vector3 P0, UnityEngine.Vector3 P1, UnityEngine.Vector3 T1, float f, float FT0, float FC0, float FB0, float FT1, float FC1, float FB1)
    {
    ...
    void doRefreshOrientation()
    {
        if (Count > 0) {
            ...
            //UnityEngine.Vector3.OrthoNormalize(ref this[0].ApproximationT[0], ref LastUp);
            for (int i = 0; i < Count; i++) {
    ...
    float AngleSigned(UnityEngine.Vector3 v1, UnityEngine.Vector3 v2, UnityEngine.Vector3 n)
    {
        return UnityEngine.Mathf.Atan2(UnityEngine.Vector3.Dot(n, UnityEngine.Vector3.Cross(v1, v2)),UnityEngine.Vector3.Dot(v1, v2)) * UnityEngine.Mathf.Rad2Deg;
    }
    ...
    UnityEngine.Vector3 minAxis(UnityEngine.Vector3 v)
    {
        return v.x < v.y ? (v.x < v.z ? new UnityEngine.Vector3(1, 0, 0) : new UnityEngine.Vector3(0, 0, 1)) : (v.y < v.z ? new UnityEngine.Vector3(0, 1, 0) : new UnityEngine.Vector3(0, 0, 1));
    }


    UnityEngine.Vector3 GetUpVector(UnityEngine.Vector3 P, UnityEngine.Vector3 P1, UnityEngine.Vector3 LastUp)
    {
        UnityEngine.Vector3 T = (P - P1).normalized;
        UnityEngine.Vector3 B = UnityEngine.Vector3.Cross(T, LastUp).normalized;
        return UnityEngine.Vector3.Cross(B, T).normalized;
    }
    ...
    float LinePointDistanceSqr(UnityEngine.Vector3 l1, UnityEngine.Vector3 l2, UnityEngine.Vector3 p, out float frag)
    {
        UnityEngine.Vector3 v = l2 - l1;
        UnityEngine.Vector3 w = p - l1;
        float c1 = UnityEngine.Vector3.Dot(w, v);
        if (c1 <= 0) {
            frag = 0;
            return (p - l1).sqrMagnitude;
        }
        float c2 = UnityEngine.Vector3.Dot(v, v);
        if (c2 <= c1) {
            frag = 1;
            return (p - l2).sqrMagnitude;
        }
        frag=c1 / c2;
        UnityEngine.Vector3 pb = l1 + frag * v;
