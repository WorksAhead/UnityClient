...
public class CurvySplineInspector : Editor {
    ...
    static void CreateCurvySpline()
    {
        ...
        spl.Closed=true;
        spl.Add(new UnityEngine.Vector3(-2, -1, 0), new UnityEngine.Vector3(0, 2, 0), new UnityEngine.Vector3(2, -1, 0));
    ...
    void OnSceneGUI()
    {
        ...
        mUserValuesLabel.normal.textColor = UnityEngine.Color.green;
        ...
        if (Target.ShowLabels) {
            ...
            stLabel.normal.textColor = UnityEngine.Color.white;
            Handles.Label(Target.Transform.position - new UnityEngine.Vector3(-0.5f, 0.2f, 0), Target.name,stLabel);
            foreach (CurvySplineSegment cp in Target.ControlPoints)
                Handles.Label(cp.Position + new UnityEngine.Vector3(-0.5f, HandleUtility.GetHandleSize(cp.Position) * 0.35f, 0), cp.name, stLabel);
        }

        Handles.EndGUI();

        if (Event.current.type == EventType.KeyDown && Event.current.keyCode == UnityEngine.KeyCode.T && Target.ControlPointCount>0) {
    ...
    void UserValueReadOut(UnityEngine.Vector3 p, UnityEngine.Vector3[] values)
    {
        string uvalues = "";
        for (int i = 0; i < values.Length; i++)
            uvalues += string.Format("{0}: {1}\n", new object[]{i, values[i]});
        Handles.Label(p + new UnityEngine.Vector3(0, -0.2f, 0), uvalues, mUserValuesLabel);
    }
    ...
    void OnShowManual()
    {
        UnityEngine.Application.OpenURL("http://docs.fluffyunderware.com/curvy");
    }

    void OnShowWeb()
    {
        UnityEngine.Application.OpenURL("http://www.fluffyunderware.com/pages/unity-plugins/curvy.php");
    }

    void OnBugReport()
    {
        UnityEngine.Application.OpenURL("mailto:bugreport@fluffyunderware.com?subject=[BUG] Curvy "+CurvySpline.Version+"&body=* Please give a brief description of the bug (please attach any screenshots or example projects that might be helpful) :%0A%0A* How to reproduce the bug:%0A%0A");
    }
