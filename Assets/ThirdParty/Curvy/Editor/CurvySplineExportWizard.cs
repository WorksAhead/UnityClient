...
public class CurvySplineExportWizard : EditorWindow {
    ...
    static public void Create()
    {
        ...
        win.maxSize = new UnityEngine.Vector2(400, 275);
    ...
    void OnGUI()
    {
        ...
        if (!Planar) {
            ...
            if (GUILayout.Button("Equalize X")) {
                Undo.RegisterUndo(EditorUtility.CollectDeepHierarchy(new UnityEngine.Object[] { Spline }), "Equalize X");
                CurvyUtility.makePlanar(Spline, 0);
            }
            if (GUILayout.Button("Equalize Y")) {
                Undo.RegisterUndo(EditorUtility.CollectDeepHierarchy(new UnityEngine.Object[] { Spline }), "Equalize Y");
                CurvyUtility.makePlanar(Spline, 1);
            }
            if (GUILayout.Button("Equalize Z")) {
                Undo.RegisterUndo(EditorUtility.CollectDeepHierarchy(new UnityEngine.Object[] { Spline }), "Equalize Z");
        ...
        if (GUILayout.Button("Create GameObject")) {
            ...
            if (msh) {
                msh.name = MeshName;
                var go = new UnityEngine.GameObject(MeshName, typeof(MeshRenderer), typeof(MeshFilter));
                go.GetComponent<MeshFilter>().sharedMesh = msh;
                go.GetComponent<MeshRenderer>().sharedMaterial = MeshMaterial;
                Selection.activeGameObject = go;
                Debug.Log("Curvy Export: UnityEngine.GameObject created!");
            }
            else
                Debug.LogWarning("Curvy Export: Unable to triangulate spline!");
    ...
    void Preview(SceneView sceneView)
    {
        ...
        Handles.color = new UnityEngine.Color(0, 0.2f, 0);
        for (int v = 0; v < Vertices.Length; v++)
            Handles.CubeCap(0, Vertices[v], UnityEngine.Quaternion.identity, HandleUtility.GetHandleSize(Vertices[v]) * 0.06f);
