...
public class CurvySplineSegmentInspector : Editor {
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (Target.Spline.Interpolation == CurvyInterpolation.Bezier) {
            ...
            UnityEngine.Vector3 v = tHandleIn.vector3Value;
    ...
    void InsBefore()
    {
        ...
        if (!Target.PreviousControlPoint && SceneIsSelected) {
            UnityEngine.Vector3 p = HandleUtility.GUIPointToWorldRay(Event.current.mousePosition).GetPoint((SceneView.currentDrawingSceneView.camera.transform.position - Target.Position).magnitude);
            if (ConstrainXAxis)
