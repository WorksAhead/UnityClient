...
public class CurvySplinePathMeshBuilderInspector : Editor
{
    ...
    public static void CreateMeshBuilder()
    {
        ...
        var prim = UnityEngine.GameObject.CreatePrimitive(PrimitiveType.Plane);
#if UNITY_3_5_7
        prim.active = false;
#else
    ...
    public override void OnInspectorGUI()
    {
        ...
        switch (tExtrusion.enumNames[tExtrusion.enumValueIndex]) {
            case "FixedF":
                tExtrusionParam.floatValue = UnityEngine.Mathf.Clamp01(tExtrusionParam.floatValue);
                paramDesc = new GUIContent("Step Width","Step width in F");
                break;
            case "FixedDistance":
                if (Target.Spline)
                    tExtrusionParam.floatValue = UnityEngine.Mathf.Clamp(tExtrusionParam.floatValue, 0, Target.Spline.Length);
        ...
        if (GUILayout.Button(new GUIContent(mTexRefresh,"Force Mesh Refresh"),GUILayout.ExpandWidth(false)))
            refresh=true;
        if (GUILayout.Button(new GUIContent(mTexDetach,"Clone to an individual UnityEngine.GameObject"),GUILayout.ExpandWidth(false))) {
