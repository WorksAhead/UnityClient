...
namespace Curvy.Utils
{
    public class CurvyEditorUtility
    {
        ...
        public static UnityEngine.Vector3 PositionHandle(int id, UnityEngine.Vector3 position, UnityEngine.Quaternion rotation, float size, bool lockX, bool lockY, bool lockZ)
        {
            ...
            UnityEngine.Vector3 snap = new UnityEngine.Vector3(EditorPrefs.GetFloat("MoveSnapX", 1f), EditorPrefs.GetFloat("MoveSnapY", 1f), EditorPrefs.GetFloat("MoveSnapZ", 1f));

            UnityEngine.Vector3 newPos = position;
            // X
            if (!lockX) {
                //GUI.SetNextControlName(name);
                Handles.color = new UnityEngine.Color(0.9f, 0.3f, 0.1f);
                newPos += Handles.Slider(position, rotation * UnityEngine.Vector3.right,sliderSize,Handles.ArrowCap,snap.x)-position;
            }
            ...
            if (!lockY) {
                //GUI.SetNextControlName(name);
                Handles.color = new UnityEngine.Color(0.6f, 0.9f, 0.3f);
                newPos += Handles.Slider(position, rotation * UnityEngine.Vector3.up, sliderSize, Handles.ArrowCap, snap.y) - position;
            }
            ...
            if (!lockZ) {
                //GUI.SetNextControlName(name);
                Handles.color = new UnityEngine.Color(0.2f, 0.4f, 0.9f);
                newPos += Handles.Slider(position, rotation * UnityEngine.Vector3.forward, sliderSize, Handles.ArrowCap, snap.z) - position;
            }
            ...
            if (!lockX && !lockY) {
                //GUI.SetNextControlName(name);
                Handles.color = new UnityEngine.Color(0.2f, 0.4f, 0.9f);
                newPos+=Handles.Slider2D(id,position, rotation*new UnityEngine.Vector3(rectSize, rectSize, 0), rotation*UnityEngine.Vector3.forward, rotation*UnityEngine.Vector3.up, rotation*UnityEngine.Vector3.right, rectSize, Handles.RectangleCap, new UnityEngine.Vector2(snap.x,snap.y))-position;            
            }
            // X/Z Slider
            if (!lockX && !lockZ) {
                //GUI.SetNextControlName(name);
                Handles.color = new UnityEngine.Color(0.6f, 0.9f, 0.3f);
                newPos += Handles.Slider2D(id - 1, position, rotation * new UnityEngine.Vector3(rectSize, 0, -rectSize), rotation * UnityEngine.Vector3.up, rotation * UnityEngine.Vector3.right, rotation * UnityEngine.Vector3.forward, rectSize, Handles.RectangleCap, new UnityEngine.Vector2(snap.x, snap.z)) - position;            
            }
            // Y/Z Slider
            if (!lockY && !lockZ) {
                //GUI.SetNextControlName(name);
                Handles.color = new UnityEngine.Color(0.9f, 0.3f, 0.1f);
                newPos += Handles.Slider2D(id - 2, position, rotation * new UnityEngine.Vector3(0, rectSize, -rectSize), rotation * UnityEngine.Vector3.left, rotation * UnityEngine.Vector3.up, rotation * UnityEngine.Vector3.forward, rectSize, Handles.RectangleCap, new UnityEngine.Vector2(snap.y, snap.z)) - position;            
            }

            return newPos;
        }

        public static T[] GetSelection<T>() where T:UnityEngine.Object
        {
            List<T> res = new List<T>();
            UnityEngine.Object[] obj=Selection.GetFiltered(typeof(T), SelectionMode.TopLevel);
            foreach (T r in obj)
                res.Add(r);
