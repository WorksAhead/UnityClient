...
public class MB_MeshBakerEditorWindow : EditorWindow
{
	...
	UnityEngine.GameObject targetGO = null;
	UnityEngine.GameObject oldTargetGO = null;
	...
	UnityEngine.Vector2 scrollPos = UnityEngine.Vector2.zero;
	...
	void addLODToSelected(){
		...
		if (numAdded == 0){
			Debug.LogWarning("Added LOD to 0 objects. Make sure some or all objects are selected in the hierarchy view. Also check ths 'Only Static Objects', 'Using Material' and 'Using UnityEngine.Shader' settings");
		} else {
	...
	public class _GameObjectAndWarning : IComparable{
		...
		public UnityEngine.Shader[] shaders = null;
...
//		Profile.StartProfile("listMaterialsInScene1");
	...
	string generateSceneAnalysisReport(Dictionary<_GameObjectAndWarning,List<_GameObjectAndWarning>> gs2bakeGroupMap, List<_GameObjectAndWarning> objsNotAddedToBaker){
		...
		if (objsNotAddedToBaker.Count > 0){
			...
			for (int i = 0; i < gos1.Count; i++){
				...
				string matStr = "";
				UnityEngine.Renderer mr = gos1[i].go.GetComponent<UnityEngine.Renderer>();
				foreach(Material mmm in mr.sharedMaterials){
					matStr += "[" + mmm + "] ";
				}				
	...
	bool ShadersAreTheSame(_GameObjectAndWarning a, _GameObjectAndWarning b){
		HashSet<UnityEngine.Shader> aMats = new HashSet<UnityEngine.Shader>();
		for(int i = 0; i < a.shaders.Length; i++) aMats.Add(a.shaders[i]);
		HashSet<UnityEngine.Shader> bMats = new HashSet<UnityEngine.Shader>();
		for(int i = 0; i < b.shaders.Length; i++) bMats.Add(b.shaders[i]);
		return aMats.SetEquals(bMats);
	}

	void sortIntoBakeGroups2(bool useFilters,
							Dictionary<UnityEngine.Shader,List<_GameObjectAndWarning>> shader2GameObjects,
...
//		HashSet<UnityEngine.GameObject> alreadyInMeshBaker = new HashSet<UnityEngine.GameObject>();
...
//			List<UnityEngine.GameObject> gos = mbsInScene[i].GetObjectsToCombine();
