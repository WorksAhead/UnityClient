...
namespace DigitalOpus.MB.Core{
	...
	//	Mesh GetMesh();
	//	bool doUV2();  
		List<UnityEngine.GameObject> GetObjectsInCombined();
		...
		void Apply(bool triangles,
							  ...
							  bool uv2,
							  bool bones=false);
		void UpdateGameObjects(UnityEngine.GameObject[] gos, bool recalcBounds = true);	
		Mesh AddDeleteGameObjects(UnityEngine.GameObject[] gos, UnityEngine.GameObject[] deleteGOs, bool disableRendererInSource, bool fixOutOfBoundUVs);
	...
	*/
	...
	public class MB2_MeshCombiner{
		...
		*/
		...
		[SerializeField] bool containsHiddenObjects = false;
		
		//this contains object instances that have been added to the combined mesh through AddDelete
		[SerializeField] List<UnityEngine.GameObject> objectsInCombinedMesh = new List<UnityEngine.GameObject>();
		[SerializeField] List<MB_DynamicGameObject> mbDynamicObjectsInCombinedMesh = new List<MB_DynamicGameObject>();
		Dictionary<UnityEngine.GameObject,MB_DynamicGameObject> _instance2combined_map = new Dictionary<UnityEngine.GameObject, MB_DynamicGameObject>();
		
		[SerializeField] UnityEngine.Vector3[] verts = new UnityEngine.Vector3[0];
		[SerializeField] UnityEngine.Vector3[] normals = new UnityEngine.Vector3[0];
		[SerializeField] Vector4[] tangents = new Vector4[0];
		[SerializeField] UnityEngine.Vector2[] uvs = new UnityEngine.Vector2[0];
		[SerializeField] UnityEngine.Vector2[] uv1s = new UnityEngine.Vector2[0];
		[SerializeField] UnityEngine.Vector2[] uv2s = new UnityEngine.Vector2[0];
		[SerializeField] UnityEngine.Color[] colors = new UnityEngine.Color[0];
		[SerializeField] Matrix4x4[] bindPoses = new Matrix4x4[0];
		[SerializeField] UnityEngine.Transform[] bones = new UnityEngine.Transform[0];
	    ...
	    UnityEngine.GameObject[] empty = new UnityEngine.GameObject[0];		
		...
		public UnityEngine.Renderer targetRenderer { 
		...
		MB_DynamicGameObject instance2Combined_MapGet(UnityEngine.GameObject key){
			return _instance2combined_map[key];
		}

		void instance2Combined_MapAdd(UnityEngine.GameObject key, MB_DynamicGameObject dgo){
			_instance2combined_map.Add(key, dgo);
		}

		void instance2Combined_MapRemove(UnityEngine.GameObject key){
			_instance2combined_map.Remove(key);
		}

		bool instance2Combined_MapTryGetValue(UnityEngine.GameObject key, out MB_DynamicGameObject dgo){
		...
		bool instance2Combined_MapContainsKey(UnityEngine.GameObject key){
		...
		public List<UnityEngine.GameObject> GetObjectsInCombined(){
			List<UnityEngine.GameObject> outObs = new List<UnityEngine.GameObject>();
	...
	//			_mesh.MarkDynamic();			
			...
			for(int i = 0; i < numTriMeshes; i++){
				...
				if (textureBakeResults.doMultiMaterial){
					if (!sourceMats2submeshIdx_map.Contains(sharedMaterials[i])){
						Debug.LogError("UnityEngine.Object " + dgo.go + " has a material that was not found in the result materials maping. " + sharedMaterials[i]);
						return false;
					}
		...
		bool getIsGameObjectActive(UnityEngine.GameObject g){
	#if UNITY_3_5
			return g.active;
	#else
		...
		bool _showHide(UnityEngine.GameObject[] goToShow, UnityEngine.GameObject[] goToHide){
			...
			for (int i = 0; i < goToShow.Length; i++){
				if( !instance2Combined_MapContainsKey(goToShow[i])){			
					Debug.LogWarning("Trying to show an object "+goToShow[i]+" that is not in combined mesh");
					return false;
				}
			}
			...
			if (goToShow.Length > 0) containsHiddenObjects = true;
 
			return true;
		}
		
		bool _addToCombined(UnityEngine.GameObject[] goToAdd, UnityEngine.GameObject[] goToDelete,bool disableRendererInSource){
			UnityEngine.GameObject[] _goToAdd, _goToDelete;
			...
			else _goToAdd = (UnityEngine.GameObject[]) goToAdd.Clone();
			if (goToDelete == null) _goToDelete = empty;
			else _goToDelete = (UnityEngine.GameObject[]) goToDelete.Clone();
			...
			for (int i = 0; i < _goToAdd.Length; i++){
				...
				if(!instance2Combined_MapContainsKey(_goToAdd[i]) || Array.Find<UnityEngine.GameObject>(_goToDelete,o => o == _goToAdd[i])){
					MB_DynamicGameObject dgo = new MB_DynamicGameObject();
	
					UnityEngine.GameObject go = _goToAdd[i];
					
					Material[] sharedMaterials = MB_Utility.GetGOMaterials(go);
	
					if (sharedMaterials == null){
						Debug.LogError("UnityEngine.Object " + go.name + " does not have a UnityEngine.Renderer");
						_goToAdd[i] = null;
						return false;
					}
					...
					if (m == null){
						Debug.LogError("UnityEngine.Object " + go.name + " MeshFilter or SkinedMeshRenderer had no mesh");
						_goToAdd[i] = null;	
						return false;
					}
					...
					for (int j = 0; j < sharedMaterials.Length; j++){
						if (!mat2rect_map.TryGetValue(sharedMaterials[j],out uvRects[j])){
							Debug.LogError("UnityEngine.Object " + go.name + " has an unknown material " + sharedMaterials[j] + ". Try baking textures");
							_goToAdd[i] = null;			
						}
					}				
					if (_goToAdd[i] != null){
						...
						UnityEngine.Renderer r = MB_Utility.GetRenderer(dgo.go);
						...
						if (lightmapOption == MB2_LightmapOptions.preserve_current_lightmapping){
							if (lightmapIndex != r.lightmapIndex){
								Debug.LogWarning("UnityEngine.Object " + go.name + " has a different lightmap index. Lightmapping will not work.");						
							}
							if (!getIsGameObjectActive(dgo.go)){
								Debug.LogWarning("UnityEngine.Object " + go.name + " is inactive. Can only get lightmap index of active objects.");													
							}
							if (r.lightmapIndex == -1){
								Debug.LogWarning("UnityEngine.Object " + go.name + " does not have an index to a lightmap.");													
							}						
						}
					...
					Debug.LogWarning("UnityEngine.Object " + _goToAdd[i].name + " has already been added");
					_goToAdd[i] = null;
				}
			}
			...
			UnityEngine.Vector3[] nnormals = null;
			Vector4[] ntangents = null;
			UnityEngine.Vector2[] nuvs = null, nuv1s = null, nuv2s = null;
			UnityEngine.Color[] ncolors = null;
			
			UnityEngine.Vector3[] nverts = new UnityEngine.Vector3[newVertSize];
			
			if (_doNorm) nnormals = new UnityEngine.Vector3[newVertSize];
			if (_doTan) ntangents = new Vector4[newVertSize];
			if (_doUV) nuvs = new UnityEngine.Vector2[newVertSize];
			if (_doUV1) nuv1s = new UnityEngine.Vector2[newVertSize];		
			if (lightmapOption == MB2_LightmapOptions.copy_UV2_unchanged ||
				lightmapOption == MB2_LightmapOptions.preserve_current_lightmapping) nuv2s = new UnityEngine.Vector2[newVertSize];
			if (_doCol) ncolors = new UnityEngine.Color[newVertSize];
			
			BoneWeight[] nboneWeights = new BoneWeight[newVertSize];
			Matrix4x4[] nbindPoses = new Matrix4x4[newBonesSize];
			UnityEngine.Transform[] nbones = new UnityEngine.Transform[newBonesSize];
			...
			for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++){
				...
				if (!dgo._beingDeleted){
					...
					if (!dgo.show) containsHiddenObjects = true;
			...
			for (int i = 0; i < toAddDGOs.Count; i++){
				...
				if (doUV2()){
					...
					if (lightmapOption == MB2_LightmapOptions.preserve_current_lightmapping){ //has a lightmap
						UnityEngine.Vector2 uvscale2;
						Vector4 lightmapTilingOffset = dgo.lightmapTilingOffset;
						UnityEngine.Vector2 uvscale = new UnityEngine.Vector2( lightmapTilingOffset.x, lightmapTilingOffset.y );	
						UnityEngine.Vector2 uvoffset = new UnityEngine.Vector2( lightmapTilingOffset.z, lightmapTilingOffset.w );
		...
		UnityEngine.Color[] _getMeshColors(Mesh m){
			UnityEngine.Color[] cs = m.colors;
			if (cs.Length == 0){
				...
				cs = new UnityEngine.Color[m.vertexCount];
				for (int i = 0; i < cs.Length; i++){cs[i] = UnityEngine.Color.white;}
			}
			return cs;
		}
	
		UnityEngine.Vector3[] _getMeshNormals(Mesh m){
			UnityEngine.Vector3[] ns = m.normals;
			if (ns.Length == 0){
				if (VERBOSE) Debug.Log("Mesh " + m + " has no normals. Generating");
				Debug.LogWarning("Mesh " + m + " didn't have normals. Generating normals.");
				Mesh tempMesh = (Mesh) UnityEngine.GameObject.Instantiate(m);
		...
		Vector4[] _getMeshTangents(Mesh m){
			...
			if (ts.Length == 0){
				...
				UnityEngine.Vector2[] uvs = _getMeshUVs(m);
				UnityEngine.Vector3[] norms = _getMeshNormals(m);
		...
		UnityEngine.Vector2 _HALF_UV = new UnityEngine.Vector2(.5f, .5f);
		UnityEngine.Vector2[] _getMeshUVs(Mesh m){
			UnityEngine.Vector2[] uv = m.uv;
			if (uv.Length == 0){
				...
				uv = new UnityEngine.Vector2[m.vertexCount];
				for (int i = 0; i < uv.Length; i++){uv[i] = _HALF_UV;}			
			}		
			return uv;		
		}
	
		UnityEngine.Vector2[] _getMeshUV1s(Mesh m){
			UnityEngine.Vector2[] uv = m.uv2;
			if (uv.Length == 0){
		...
		UnityEngine.Vector2[] _getMeshUV2s(Mesh m){
			UnityEngine.Vector2[] uv = m.uv2;
			if (uv.Length == 0){
		...
		public void UpdateSkinnedMeshApproximateBounds(){
			...
			if (targetRenderer == null){
				Debug.LogWarning("Target UnityEngine.Renderer is not set. No point in calling UpdateSkinnedMeshApproximateBounds.");
				return;			
			}
			if (!__targetRenderer.GetType().Equals( typeof(SkinnedMeshRenderer) )){
				Debug.LogWarning("Target UnityEngine.Renderer is not a SkinnedMeshRenderer. No point in calling UpdateSkinnedMeshApproximateBounds.");
				return;			
			}
			...
			for (int i = 1; i < bones.Length; i++){
				UnityEngine.Vector3 v = bones[i].position;
			...
			UnityEngine.Vector3 center = (max + min)/2f;
			UnityEngine.Vector3 size = max - min;
		...
		int _getNumBones(UnityEngine.Renderer r){
			if (renderType == MB_RenderType.skinnedMeshRenderer){
				if (r is SkinnedMeshRenderer){
					...
					Debug.LogError("Could not _getNumBones. UnityEngine.Object does not have a renderer");
					return 0;
				}
			} else {
				return 0;	
		...
		UnityEngine.Transform[] _getBones(UnityEngine.Renderer r){
			if (r is SkinnedMeshRenderer){
				return ((SkinnedMeshRenderer)r).bones;	
			} else if (r is MeshRenderer){
				UnityEngine.Transform[] bone = new UnityEngine.Transform[1];
				bone[0] = r.transform;
				return bone;
			} else {
				Debug.LogError("Could not getBones. UnityEngine.Object does not have a renderer");
				return null;
			}
		}
		
		Matrix4x4[] _getBindPoses(UnityEngine.Renderer r){
			if (r is SkinnedMeshRenderer){
				...
				Debug.LogError("Could not _getBindPoses. UnityEngine.Object does not have a renderer");
				return null;
			}		
		}
		
		BoneWeight[] _getBoneWeights(UnityEngine.Renderer r,int numVerts){
			if (r is SkinnedMeshRenderer){
				...
				Debug.LogError("Could not _getBoneWeights. UnityEngine.Object does not have a renderer");
				return null;
			}			
		}
		...
		void _updateGameObjects(UnityEngine.GameObject[] gos, bool recalcBounds){
			_initialize();		
		...
		void _updateGameObject(UnityEngine.GameObject go, bool recalcBounds){
				MB_DynamicGameObject dgo = null;
				if (!instance2Combined_MapTryGetValue(go,out dgo)){
					Debug.LogError("UnityEngine.Object " + go.name + " has not been added");
					return;
				}
				Mesh mesh = dgo.sharedMesh;
				if (dgo.numVerts != mesh.vertexCount){
					Debug.LogError("UnityEngine.Object " + go.name + " source mesh has been modified since being added");
					return;			
				}
				Matrix4x4 l2wMat = go.transform.localToWorldMatrix;
				UnityEngine.Quaternion l2wQ = go.transform.rotation;
	
				UnityEngine.Vector3[] nverts = mesh.vertices;
				UnityEngine.Vector3[] nnorms = mesh.normals;
		...
		public Mesh AddDeleteGameObjects(UnityEngine.GameObject[] gos, UnityEngine.GameObject[] deleteGOs){
			return AddDeleteGameObjects(gos,deleteGOs,true,textureBakeResults.fixOutOfBoundsUVs);
		}
		
		public Mesh AddDeleteGameObjects(UnityEngine.GameObject[] gos, UnityEngine.GameObject[] deleteGOs, bool disableRendererInSource){		
			return AddDeleteGameObjects(gos,deleteGOs,disableRendererInSource,textureBakeResults.fixOutOfBoundsUVs);
		}
		
		public bool ShowHideGameObjects(UnityEngine.GameObject[] toShow, UnityEngine.GameObject[] toHide){
			return _showHide(toShow,toHide);	
		}
		
		public Mesh AddDeleteGameObjects(UnityEngine.GameObject[] gos, UnityEngine.GameObject[] deleteGOs, bool disableRendererInSource, bool fixOutOfBoundUVs){
			...
			if (gos != null){
				for (int i = 0; i < gos.Length; i++){
					...
					for (int j = i + 1; j < gos.Length; j++){
						if (gos[i] == gos[j]){
							Debug.LogError("UnityEngine.GameObject " + gos[i] + "appears twice in list of game objects to add");
							return null;
						}
					}
				}
	...
	//				}				
					for (int j = i + 1; j < deleteGOs.Length; j++){
						if (deleteGOs[i] == deleteGOs[j] && deleteGOs[i] != null){
							Debug.LogError("UnityEngine.GameObject " + deleteGOs[i] + "appears twice in list of game objects to delete");
							return null;
						}
					}
				}
		...
		public bool CombinedMeshContains(UnityEngine.GameObject go){
			return objectsInCombinedMesh.Contains(go);
		}
		
		void _clearArrays(){
			verts = new UnityEngine.Vector3[0];
			normals = new UnityEngine.Vector3[0];
			tangents = new Vector4[0];
			uvs = new UnityEngine.Vector2[0];
			uv1s = new UnityEngine.Vector2[0];
			uv2s = new UnityEngine.Vector2[0];
	    ...
	    void _generateTangents(int[] triangles, UnityEngine.Vector3[] verts, UnityEngine.Vector2[] uvs, UnityEngine.Vector3[] normals, Vector4[] outTangents){
	        ...
	        for(int a = 0; a < triangleCount; a+=3)
	        {
	            ...
	            UnityEngine.Vector3 v1 = verts[i1];
	            UnityEngine.Vector3 v2 = verts[i2];
	            UnityEngine.Vector3 v3 = verts[i3];
	
	            UnityEngine.Vector2 w1 = uvs[i1];
	            UnityEngine.Vector2 w2 = uvs[i2];
	            UnityEngine.Vector2 w3 = uvs[i3];
	        ...
	        {
	            UnityEngine.Vector3 n = normals[a];
	            ...
	            outTangents[a].w = (UnityEngine.Vector3.Dot(UnityEngine.Vector3.Cross(n, t), tan2[a]) < 0.0f) ? -1.0f : 1.0f;
	        }
	    }	
		...
		public void RebuildPrefab(UnityEngine.GameObject prefabRoot){
	#if UNITY_EDITOR
			if (EVAL_VERSION) return;
			UnityEngine.GameObject rootGO = (UnityEngine.GameObject) PrefabUtility.InstantiatePrefab(prefabRoot);
			this.buildSceneMeshObject(rootGO,_mesh,true);		
			string prefabPth = AssetDatabase.GetAssetPath(prefabRoot);
			PrefabUtility.ReplacePrefab(rootGO,AssetDatabase.LoadAssetAtPath(prefabPth,typeof(UnityEngine.GameObject)),ReplacePrefabOptions.ConnectToPrefab);
		...
		public UnityEngine.GameObject buildSceneMeshObject(UnityEngine.GameObject root, Mesh m, bool createNewChild=false){
			MB2_MeshCombiner mom = this;
			UnityEngine.GameObject meshGO;
			MeshFilter mf = null;
			MeshRenderer mr = null;
			SkinnedMeshRenderer smr = null;
			UnityEngine.Transform mt = null;
			if (!createNewChild){
			...
			if (mt == null){
				meshGO = new UnityEngine.GameObject(mom.name + "-mesh");
			} else {
			...
			List<UnityEngine.GameObject> gos = mom.GetObjectsInCombined();
