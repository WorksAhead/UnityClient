...
{
    half3 albedo = _Color.rgb * Texture2DGamma (_MainTex, texcoords.xy).rgb;
#if _DETAIL
    #if (SHADER_TARGET < 30)
    ...
    #endif
    half3 detailAlbedo = Texture2DGamma (_DetailAlbedoMap, texcoords.zw).rgb;
    #if _DETAIL_MULX2
...
#ifdef _SPECGLOSSMAP
    #if defined(_SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A)
        sg.rgb = Texture2DGamma(_SpecGlossMap, uv).rgb;
        sg.a = Texture2DGamma(_MainTex, uv).a;
    #else
        sg = Texture2DGamma(_SpecGlossMap, uv);
    #endif
    sg.a *= _GlossMapScale;
#else
    sg.rgb = _SpecColor.rgb;
    #ifdef _SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A
        sg.a = Texture2DGamma(_MainTex, uv).a * _GlossMapScale;
    #else
        sg.a = _Glossiness;
    #endif
#endif
...
half2 MetallicGloss(float2 uv)
{
    ...
    #ifdef _SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A
        mg.r = Texture2DGamma(_MetallicGlossMap, uv).r;
        mg.g = Texture2DGamma(_MainTex, uv).a;
    #else
        mg = Texture2DGamma(_MetallicGlossMap, uv).ra;
    #endif
    mg.g *= _GlossMapScale;
#else
    mg.r = _Metallic;
    #ifdef _SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A
        mg.g = Texture2DGamma(_MainTex, uv).a * _GlossMapScale;
    #else
        mg.g = _Glossiness;
    #endif
#endif
...
half3 Emission(float2 uv)
{
    ...
    return 0;
#else
    return Texture2DGamma(_EmissionMap, uv).rgb * _EmissionColor.rgb;
#endif
}
